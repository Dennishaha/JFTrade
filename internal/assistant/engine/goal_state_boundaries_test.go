...
func TestGoalWorkflowStateBoundariesFailClosedAndRemainResumable(t *testing.T) {
	...
	t.Run("continuation honors a concurrent pause and interrupted call filtering only removes internal calls", func(t *testing.T) {
		...
		if !interruptedGoalWorkflowToolCall(parent, ToolCall{ToolName: workflowTaskClaimTool, Status: "FAILED", Error: &pauseErr}) {
		...
		if !interruptedGoalWorkflowToolCall(parent, ToolCall{ToolName: workflowTaskClaimTool, Status: "FAILED", Error: &interrupted}) {
			t.Fatal("GO-ADK interruption sentinel must hide the interrupted workflow call")
		}
		if !errors.Is(errorFromSerializedADKText(interrupted), adkworkflow.ErrNodeInterrupted) {
