...
func workflowWriteInputSchema(name string) map[string]any {
	properties := map[string]any{
		...
		"workMode":          map[string]any{"type": "string", "enum": []string{"chat", "loop"}},
		"providerId":        map[string]any{"type": "string"},
		"model":             map[string]any{"type": "string"},
		"permissionMode":    map[string]any{"type": "string", "enum": []string{"approval", "less_approval", "all"}},
...
func workflowRunsListInputSchema() map[string]any {
	return strictObjectSchema(map[string]any{
		...
		"status": map[string]any{"type": "string", "enum": []string{
			...
			"PENDING_APPROVAL", "FAILED", "CANCELLED", "SKIPPED",
		}},
		"limit":  pageLimitSchema(),
		"offset": pageOffsetSchema(),
	}, nil)
...
func workflowStatusSchema() map[string]any {
	return map[string]any{"type": "string", "enum": []string{"ENABLED", "DISABLED"}}
}

func workflowTriggerStatusSchema() map[string]any {
	return map[string]any{"type": "string", "enum": []string{"ENABLED", "DISABLED", "ERROR"}}
}

func workflowToolTriggerTypeSchema() map[string]any {
	return map[string]any{"type": "string", "enum": []string{"manual", "schedule", "event", "market_threshold"}, "description": "Webhook 触发器必须通过 UI/API 创建。"}
}
