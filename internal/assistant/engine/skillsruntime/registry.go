...
func NewSkillRegistry(skillsPath string) *SkillRegistry {
	registry := NewSkillRegistryFromPath(skillsPath)
...
// builtin skills. It is used by the engine facade for direct-path constructs
// that preserve existing on-disk state.
func NewSkillRegistryFromPath(skillsPath string) *SkillRegistry {
...
func (r *SkillRegistry) Source(ctx context.Context, names []string) (adkskill.Source, error) {
	...
	allowed := jfadkmodel.NormalizeStringSlice(names)
	if len(allowed) == 0 {
		return nil, nil
	}
...
func (r *SkillRegistry) InstallURL(ctx context.Context, rawURL string) (Skill, error) {
	...
	client := providers.NewSafeHTTPClient(20*time.Second, SkillInstallHostValidator)
