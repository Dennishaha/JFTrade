...
func TestResumedExecutionFailurePersistenceIsObservable(t *testing.T) {
	ctx := t.Context()
...
func TestDirectResumeSurfacesRehydrationFailure(t *testing.T) {
	ctx := t.Context()
	runtime := newTestRuntime(t)
	agent := mustSaveAgent(t, runtime, AgentWriteRequest{
...
func TestChildApprovalResumeFallsBackToDirectRecovery(t *testing.T) {
	ctx := t.Context()
	runtime := newTestRuntime(t)
	agent := mustSaveAgent(t, runtime, AgentWriteRequest{
