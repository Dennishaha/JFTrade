...
func projectedToolResponse(state *projectedRunState, response *genai.FunctionResponse, timestamp string) {
	...
	if errorValue, ok := response.Response["error"]; ok {
		responseErr := errorFromSerializedADKValue(errorValue)
		if errors.Is(responseErr, adktool.ErrConfirmationRequired) {
			call.Status = "PENDING_APPROVAL"
			call.RequiresUser = true
			return
		}
		if errors.Is(responseErr, adkworkflow.ErrNodeInterrupted) {
