...
func TestExecutionClaimValidationAndReleaseLifecycle(t *testing.T) {
	...
	if err := nilStore.ReleaseRunLease(ctx, enginepersistence.RunLease{}); err != nil {
	...
	if _, err := nilStore.ClaimToolInvocation(ctx, enginepersistence.ToolInvocationClaim{}); err == nil {
	...
	if err := store.ReleaseRunLease(ctx, enginepersistence.RunLease{}); err != nil {
...
func TestToolInvocationAbandonAndCorruptionBoundaries(t *testing.T) {
	...
	base := enginepersistence.ToolInvocationClaim{
	...
	invalidClaims := []enginepersistence.ToolInvocationClaim{
		{},
	...
	if err := store.AbandonToolInvocation(ctx, ticket); !errors.Is(err, enginepersistence.ErrToolInvocationLost) {
	...
	if _, err := store.DB().ExecContext(ctx, `UPDATE `+enginepersistence.TableToolInvocations+` SET output_json = '{' WHERE run_id = ? AND idempotency_key = ?`, ticket.RunID, ticket.IdempotencyKey); err != nil {
	...
	if _, err := store.DB().ExecContext(ctx, `UPDATE `+enginepersistence.TableToolInvocations+` SET status = 'UNKNOWN' WHERE run_id = ? AND idempotency_key = ?`, ticket.RunID, ticket.IdempotencyKey); err != nil {
	...
	if _, err := store.ClaimToolInvocation(ctx, base); !errors.Is(err, enginepersistence.ErrToolOutcomeUnknown) {
		t.Fatalf("indeterminate replay error = %v", err)
	}
}

func TestExecutionClaimUpdateResultFailures(t *testing.T) {
	claim := enginepersistence.ToolInvocationClaim{RunID: "run", ToolName: "tool"}
	forced := errors.New("forced result failure")
	if err := enginepersistence.RequireToolClaimUpdate(nil, forced, claim); !errors.Is(err, forced) {
		t.Fatalf("direct update error = %v", err)
	}
	if err := enginepersistence.RequireToolClaimUpdate(executionClaimResultStub{err: forced}, nil, claim); !errors.Is(err, forced) {
		t.Fatalf("RowsAffected error = %v", err)
	}
	if err := enginepersistence.RequireToolClaimUpdate(executionClaimResultStub{}, nil, claim); !errors.Is(err, enginepersistence.ErrToolInvocationInFlight) {
		t.Fatalf("zero-row update error = %v", err)
	}
	if err := enginepersistence.RequireToolClaimUpdate(executionClaimResultStub{rows: 1}, nil, claim); err != nil {
		t.Fatalf("one-row update: %v", err)
	}

	tx := executionClaimTxStub{execErr: forced}
	if err := enginepersistence.LockRunLease(t.Context(), tx, "run", "owner", 1, time.Now()); !errors.Is(err, forced) {
	...
	if err := enginepersistence.LockRunLease(t.Context(), tx, "run", "owner", 1, time.Now()); !errors.Is(err, enginepersistence.ErrRunLeaseLost) {
...
func TestExecutionClaimsSurfaceClosedDatabaseFailures(t *testing.T) {
	...
	now := time.Now().UTC()
	lease := enginepersistence.RunLease{RunID: "run-closed", OwnerID: "owner", FencingToken: 1, ExpiresAt: now.Add(time.Minute)}
