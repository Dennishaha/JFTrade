...
func TestStoreDataFailureAndNormalizationBoundaries(t *testing.T) {
	ctx := t.Context()
	...
	t.Run("task patches validate status and normalize child execution metadata", func(t *testing.T) {
		...
		if _, err := store.UpdateTask(ctx, task.ID, TaskPatchRequest{Status: &invalidStatus}); !errors.Is(err, ErrInvalidTaskStatus) || !strings.Contains(err.Error(), "invalid task status") {
...
func TestStoreEntityDefaultsAndStorageFailures(t *testing.T) {
	ctx := t.Context()
