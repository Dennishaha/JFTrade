...
// NewGoogleADKArtifactService opens or initializes the SQLite-backed ADK
// artifact store. An empty path returns an in-memory ADK service.
...
// DeriveGoogleADKArtifactPathFromSessionService returns the sibling artifact
// database path for a SQLite-backed session service.
...
// Compact checkpoints and vacuums the artifact database. It is a no-op when
// the service is unavailable or backed by the in-memory implementation.
func (s *googleADKArtifactService) Compact(ctx context.Context) error {
	if s == nil || s.db == nil {
