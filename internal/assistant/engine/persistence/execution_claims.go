...
const (
	...
	TableRunLeases       = tableRunLeases
	TableToolInvocations = tableToolInvocations
)

// ClaimStore owns SQLite-backed execution leases and tool invocation claims.
// The engine composition root embeds it so orchestration can keep using the
// promoted methods without reaching into database internals.
type ClaimStore struct {
	db *sqliteconn.DB
}

func NewClaimStore(db *sqliteconn.DB) *ClaimStore {
...
func (s *ClaimStore) ClaimRunLease(
	ctx context.Context,
	runID string,
	ownerID string,
	now time.Time,
...
func (s *ClaimStore) HeartbeatRunLease(ctx context.Context, lease RunLease, now time.Time, ttl time.Duration) (RunLease, error) {
	if ttl <= 0 {
		return RunLease{}, fmt.Errorf("ADK run lease TTL must be positive")
	}
	now = now.UTC()
...
func (s *ClaimStore) ReleaseRunLease(ctx context.Context, lease RunLease) error {
...
func (s *ClaimStore) RunLease(ctx context.Context, runID string) (RunLease, bool, error) {
	if s == nil || s.db == nil {
...
func (s *ClaimStore) ClaimToolInvocation(ctx context.Context, claim ToolInvocationClaim) (ToolInvocationTicket, error) {
...
// RequireToolClaimUpdate is the exported boundary used by engine tests that
// exercise result-count semantics without opening a database.
func RequireToolClaimUpdate(result sql.Result, err error, claim ToolInvocationClaim) error {
	return requireToolClaimUpdate(result, err, claim)
}

func (s *ClaimStore) HeartbeatToolInvocation(ctx context.Context, ticket ToolInvocationTicket, now time.Time, ttl time.Duration) error {
	if ttl <= 0 {
...
func (s *ClaimStore) CompleteToolInvocation(ctx context.Context, ticket ToolInvocationTicket, output map[string]any, now time.Time) error {
...
func (s *ClaimStore) MarkToolInvocationIndeterminate(ctx context.Context, ticket ToolInvocationTicket, now time.Time) error {
	now = now.UTC()
	if now.IsZero() {
		now = time.Now().UTC()
	}
...
func (s *ClaimStore) AbandonToolInvocation(ctx context.Context, ticket ToolInvocationTicket) error {
