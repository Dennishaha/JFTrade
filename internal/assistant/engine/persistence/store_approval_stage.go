...
type approvalStage struct {
	tx           *sqliteconn.Tx
	normalizeRun func(jfadkmodel.Run) jfadkmodel.Run
}

func (s *StoreCore) beginApprovalStage(ctx context.Context) (*approvalStage, error) {
	tx, err := s.DB().BeginWrite(ctx, nil)
	if err != nil {
		return nil, err
	}
	return &approvalStage{tx: tx, normalizeRun: s.normalizeRun}, nil
}
...
func (s *approvalStage) resolveApproval(ctx context.Context, approvalID, status string) (jfadkmodel.Approval, bool, bool, error) {
	resolvedAt := jfadkmodel.NowString()
	var payload string
	changed := true
	err := s.tx.QueryRowxContext(ctx, `UPDATE `+tableApprovals+` SET status = ?, payload_json = json_set(payload_json, '$.status', ?, '$.updatedAt', ?), updated_at = ? WHERE id = ? AND status = ? RETURNING payload_json`,
		status, status, resolvedAt, resolvedAt, approvalID, jfadkmodel.ApprovalStatusPending,
	).Scan(&payload)
	...
	if errors.Is(err, sql.ErrNoRows) {
		return jfadkmodel.Approval{}, false, false, nil
	}
	if err != nil {
		return jfadkmodel.Approval{}, false, false, err
	}
...
func (s *approvalStage) lockPendingRun(ctx context.Context, runID string) error {
	...
	_, err := s.tx.ExecContext(ctx, `UPDATE `+tableRuns+` SET updated_at = updated_at WHERE id = ? AND status = ?`, runID, jfadkmodel.RunStatusPending)
	return err
}

func (s *approvalStage) pendingRun(ctx context.Context, runID string) (jfadkmodel.Run, bool, error) {
	var payload string
	if err := s.tx.QueryRowxContext(ctx, `SELECT payload_json FROM `+tableRuns+` WHERE id = ?`, runID).Scan(&payload); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return jfadkmodel.Run{}, false, nil
		}
	...
	return run, run.Status == jfadkmodel.RunStatusPending, nil
}

func (s *approvalStage) mergeApprovalStates(ctx context.Context, run *jfadkmodel.Run, resolved jfadkmodel.Approval) (bool, bool, error) {
	rows := []struct {
	...
	authoritative := make(map[string]jfadkmodel.Approval, len(rows))
	for _, row := range rows {
		var item jfadkmodel.Approval
	...
	for index := range run.PendingApprovals {
		...
		denied = denied || item.Status == jfadkmodel.ApprovalStatusDenied
	}
	return replaced, denied, nil
}

func (s *approvalStage) denyPendingSiblings(ctx context.Context, run *jfadkmodel.Run) error {
	for index := range run.PendingApprovals {
		item := &run.PendingApprovals[index]
		if item.Status != jfadkmodel.ApprovalStatusPending {
			continue
		}
		item.Status = jfadkmodel.ApprovalStatusDenied
		item.UpdatedAt = jfadkmodel.NowString()
		payload, err := json.Marshal(*item)
		if err != nil {
			return err
		}
		if _, err := s.tx.ExecContext(ctx, `UPDATE `+tableApprovals+` SET status = ?, payload_json = ?, updated_at = ? WHERE id = ? AND status = ?`,
			item.Status, string(payload), item.UpdatedAt, item.ID, jfadkmodel.ApprovalStatusPending,
		); err != nil {
			return err
		}
	}
...
func prepareApprovalContinuation(run *jfadkmodel.Run, denied bool) bool {
	shouldContinue := !jfadkmodel.RunHasPendingApproval(run.PendingApprovals)
	if !shouldContinue {
		return false
	}
	...
	run.StartedAt = jfadkmodel.NowString()
	...
	for index := range run.ToolCalls {
		...
		call.Status = "RUNNING"
		call.RequiresUser = false
		call.UpdatedAt = jfadkmodel.NowString()
	}
...
func (s *approvalStage) savePendingRun(ctx context.Context, run *jfadkmodel.Run) error {
	*run = s.normalizeRun(*run)
	...
	result, err := s.tx.ExecContext(ctx, `UPDATE `+tableRuns+` SET status = ?, payload_json = ?, updated_at = ? WHERE id = ? AND status = ?`,
		run.Status, string(payload), run.UpdatedAt, run.ID, jfadkmodel.RunStatusPending,
	)
	if err != nil {
		return err
	}
