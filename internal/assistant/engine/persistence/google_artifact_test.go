...
func TestGoogleADKArtifactServiceStoresVersionedArtifacts(t *testing.T) {
	...
	first, err := service.Save(ctx, &adkartifact.SaveRequest{
		AppName: "artifact-agent", UserID: "jftrade-user", SessionID: "session-artifact", FileName: "report.txt",
	...
	versionOne, err := service.Load(ctx, &adkartifact.LoadRequest{
		AppName: "artifact-agent", UserID: "jftrade-user", SessionID: "session-artifact", FileName: "report.txt", Version: first.Version,
	})
	if err != nil {
	...
	list, err := service.List(ctx, &adkartifact.ListRequest{
		AppName: "artifact-agent", UserID: "jftrade-user", SessionID: "session-artifact",
	})
	if err != nil {
		t.Fatalf("List: %v", err)
	}
...
func TestGoogleADKArtifactServicePersistsAcrossRestartAndUserScope(t *testing.T) {
	...
	if _, err := service.Save(ctx, &adkartifact.SaveRequest{
		AppName: "artifact-agent", UserID: "jftrade-user", SessionID: "session-a", FileName: "user:profile.txt",
		Part: genai.NewPartFromBytes([]byte("profile"), "text/custom"),
	}); err != nil {
		t.Fatalf("Save user artifact: %v", err)
	}
	if _, err := service.Save(ctx, &adkartifact.SaveRequest{
		AppName: "artifact-agent", UserID: "jftrade-user", SessionID: "session-a", FileName: "local.txt",
	...
	list, err := reopened.List(ctx, &adkartifact.ListRequest{
		AppName: "artifact-agent", UserID: "jftrade-user", SessionID: "session-b",
	})
	if err != nil {
	...
	loaded, err := reopened.Load(ctx, &adkartifact.LoadRequest{
		AppName: "artifact-agent", UserID: "jftrade-user", SessionID: "session-b", FileName: "user:profile.txt",
	})
	if err != nil {
	...
	meta, err := reopened.GetArtifactVersion(ctx, &adkartifact.GetArtifactVersionRequest{
		AppName: "artifact-agent", UserID: "jftrade-user", SessionID: "session-b", FileName: "user:profile.txt", Version: 1,
	})
	if err != nil {
