...
var ErrCleanupCandidatesChanged = jfadkmodel.ErrCleanupCandidatesChanged
...
func (s *StoreCore) PurgeDeletedConfigs(ctx context.Context, ids DeletedConfigIDs) (int, error) {
	if s == nil || s.db == nil {
	...
	for _, id := range ids.Workflows {
		...
		if err := tx.QueryRowContext(ctx, `SELECT COALESCE(json_extract(payload_json, '$.deletedAt'), '') <> '' FROM `+tableWorkflows+` WHERE id = ?`, trimmedID).Scan(&markedDeleted); err != nil || !markedDeleted {
			return 0, jfadkmodel.ErrCleanupCandidatesChanged
		}
...
func (s *StoreCore) HasDatabaseActivity(ctx context.Context) (bool, error) {
	if s == nil || s.db == nil {
		return false, nil
	}
	var count int
...
func (s *StoreCore) CompactDatabase(ctx context.Context) error {
	if s == nil || s.db == nil {
