...
func TestWorkflowPlannerArgumentAndDependencyHelpers(t *testing.T) {
	if got := assistantmodel.PlannerIntArg(map[string]any{"order": int64(7)}, "order"); got != 7 {
		t.Fatalf("plannerIntArg int64 = %d", got)
	}
	if got := assistantmodel.PlannerIntArg(map[string]any{"order": float32(3.9)}, "order"); got != 3 {
		t.Fatalf("plannerIntArg float32 = %d", got)
	}
	if got := assistantmodel.PlannerIntArg(map[string]any{"order": "not-number"}, "order"); got != 0 {
		t.Fatalf("plannerIntArg bad string = %d", got)
	}
	if got := assistantmodel.PlannerStringArg(map[string]any{"title": nil}, "title"); got != "" {
