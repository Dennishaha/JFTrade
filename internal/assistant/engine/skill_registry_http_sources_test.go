...
func TestSkillRegistryHTTPAndSourceBranches(t *testing.T) {
	...
	t.Run("InstallURL validates hosts statuses payload size and installed content", func(t *testing.T) {
		...
		oldValidator := skillsruntime.SkillInstallHostValidator
		skillsruntime.SkillInstallHostValidator = func(context.Context, string) error { return errors.New("blocked host") }
		...
		skillsruntime.SkillInstallHostValidator = oldValidator
		t.Cleanup(func() { skillsruntime.SkillInstallHostValidator = oldValidator })
		...
		largeServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
			_, _ = w.Write([]byte(strings.Repeat("x", skillsruntime.MaxSkillFileSize+1)))
		}))
...
func TestSkillInstallDeterministicErrorBranches(t *testing.T) {
	ctx := context.Background()
