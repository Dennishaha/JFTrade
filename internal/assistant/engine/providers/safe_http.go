...
// NewSafeHTTPClient returns an HTTP client whose dial path re-validates every
// resolved address against the supplied host validator.
func NewSafeHTTPClient(timeout time.Duration, validate func(context.Context, string) error) *http.Client {
	return newSafeHTTPClient(timeout, validate)
}
