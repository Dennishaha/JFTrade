...
// LegacyAssistantContentSplitter exposes the compatibility splitter to
// consumers outside this package without leaking the internal state fields.
type LegacyAssistantContentSplitter = legacyAssistantContentSplitter

// ReasoningMode is the visible counterpart of the internal splitter mode.
type ReasoningMode = reasoningMode

const (
	ReasoningModeReply     = reasoningModeReply
	ReasoningModeReasoning = reasoningModeReasoning
)

// NewLegacyAssistantContentSplitter constructs a splitter in the requested
// mode. Callers that only need the default reply-first behavior can use the
// zero value of LegacyAssistantContentSplitter.
func NewLegacyAssistantContentSplitter(mode ReasoningMode) *LegacyAssistantContentSplitter {
	return &legacyAssistantContentSplitter{mode: mode}
}

// SplitLegacyAssistantContent splits one complete content string into visible
// reply and reasoning text.
func SplitLegacyAssistantContent(content string) (string, string) {
	return splitLegacyAssistantContent(content)
}

// ExtractVisibleAndReasoningText prefers native reasoning fields and falls
// back to the legacy tag splitter.
func ExtractVisibleAndReasoningText(content string, reasoningParts ...string) (string, string) {
	return extractVisibleAndReasoningText(content, reasoningParts...)
}
