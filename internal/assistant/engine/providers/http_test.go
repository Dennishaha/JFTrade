...
func TestHTTPClientAllowsPrivateNetworkProvider(t *testing.T) {
	...
	response, err := NewHTTPClient(time.Second).Do(req)
	if err != nil {
...
func TestHTTPClientRevalidatesRedirectDNSResolution(t *testing.T) {
	var server *httptest.Server
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		parsed, err := url.Parse(serverURLWithHost(t, "rebound.test", serverPort(t, server)))
		if err != nil {
	...
	client := NewHTTPClientWithResolver(time.Second, lookup)
	req, err := http.NewRequestWithContext(
		t.Context(),
		http.MethodGet,
		serverURLWithHost(t, "model.test", serverPort(t, server)),
		nil,
	)
	if err != nil {
		t.Fatalf("new request: %v", err)
