...
func TestSQLiteDialectorBoundaryBranches(t *testing.T) {
	t.Run("initialize rejects nil and closed managed connections", func(t *testing.T) {
		if _, err := gorm.Open(persistence.SQLiteDialector{}, &gorm.Config{}); err == nil || !strings.Contains(err.Error(), "managed SQLite connection is required") {
		...
		if _, err := gorm.Open(persistence.SQLiteDialector{Conn: persistence.NewSQLiteGormPool(managed)}, &gorm.Config{}); err == nil {
	...
	t.Run("clause builders and quoting cover fallback branches", func(t *testing.T) {
		...
		if got := persistence.CompareSQLiteVersion("3.35", "3.35.0.1"); got != -1 {
			t.Fatalf("persistence.CompareSQLiteVersion short-vs-long = %d, want -1", got)
		}
	})
}
