...
func TestAnsweredInputFailuresRemainObservable(t *testing.T) {
	ctx := t.Context()
...
func TestInputResolutionExposesParentProjectionWriteFailure(t *testing.T) {
	ctx := t.Context()
	runtime, agent, session, child := newPendingInputRun(t, "input-parent-projection")
...
func newAnsweredInputRun(t *testing.T, suffix string) (*Runtime, Agent, Session, Run) {
	t.Helper()
	runtime, agent, session, run := newPendingInputRun(t, suffix)
...
func newPendingInputRun(t *testing.T, suffix string) (*Runtime, Agent, Session, Run) {
	t.Helper()
	runtime := newTestRuntime(t)
	agent := mustSaveAgent(t, runtime, AgentWriteRequest{
