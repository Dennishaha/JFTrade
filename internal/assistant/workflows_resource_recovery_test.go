...
func TestWorkflowResourcesRejectCrossWorkflowAndInvalidRequests(t *testing.T) {
	runtime, service, _ := newAssistantServiceHarness(t)
	ctx := t.Context()
	firstAgent, first := saveWorkflowTestAgentAndDefinition(t, runtime, service, "coverage98-workflow-first", assistantmodel.WorkflowStatusEnabled)
	_, second := saveWorkflowTestAgentAndDefinition(t, runtime, service, "coverage98-workflow-second", assistantmodel.WorkflowStatusEnabled)
	trigger, err := service.SaveWorkflowTrigger(ctx, first.ID, "", assistantmodel.WorkflowTriggerWriteRequest{
		ID: "coverage98-workflow-first-trigger", Type: assistantmodel.WorkflowTriggerTypeManual, Status: assistantmodel.WorkflowTriggerStatusEnabled,
	})
	if err != nil {
		t.Fatalf("SaveWorkflowTrigger: %v", err)
	}

	if _, err := service.SaveWorkflow(ctx, "coverage98-workflow-missing", assistantmodel.WorkflowDefinitionWriteRequest{}); err == nil || !strings.Contains(err.Error(), "workflow not found") {
		t.Fatalf("SaveWorkflow missing update error = %v", err)
	}
	if _, err := service.SaveWorkflow(ctx, "", assistantmodel.WorkflowDefinitionWriteRequest{
		ID: "coverage98-workflow-invalid-mode", Name: "Invalid Mode", Status: assistantmodel.WorkflowStatusDisabled,
	...
	if _, err := service.SaveWorkflowTrigger(ctx, second.ID, trigger.Trigger.ID, assistantmodel.WorkflowTriggerWriteRequest{Type: assistantmodel.WorkflowTriggerTypeManual}); err == nil || !strings.Contains(err.Error(), "workflow trigger not found") {
...
func TestWorkflowAsyncTriggerAndBackgroundRecovery(t *testing.T) {
	ctx := t.Context()
	...
	t.Run("accepted trigger runs asynchronously with a detached trigger copy", func(t *testing.T) {
		...
		_, workflow := saveWorkflowTestAgentAndDefinition(t, runtime, service, "coverage98-workflow-async", assistantmodel.WorkflowStatusEnabled)
		trigger, err := service.SaveWorkflowTrigger(ctx, workflow.ID, "", assistantmodel.WorkflowTriggerWriteRequest{
			ID: "coverage98-workflow-async-trigger", Type: assistantmodel.WorkflowTriggerTypeManual, Status: assistantmodel.WorkflowTriggerStatusEnabled,
		})
		if err != nil {
			t.Fatalf("SaveWorkflowTrigger: %v", err)
		}
		...
		finished := waitForWorkflowRunStatus(t, runtime, started.Log.ID, assistantmodel.WorkflowTriggerLogStatusSucceeded)
	...
	t.Run("template failures happen before runtime access", func(t *testing.T) {
		workflow := assistantmodel.WorkflowDefinition{CanvasGraph: &assistantmodel.WorkflowCanvasGraph{Nodes: []assistantmodel.WorkflowCanvasNode{{
	...
	t.Run("background panic is converted into a durable failed workflow log", func(t *testing.T) {
		...
		workflow := assistantmodel.WorkflowDefinition{
			ID: "coverage98-workflow-background-panic", Name: "Background Panic", Status: assistantmodel.WorkflowStatusEnabled,
			AgentID: "coverage98-agent", PromptTemplate: "render before service access",
		}
		var unavailable *Service
		unavailable.executeQueuedWorkflowBackground(context.Background(), store, workflow, nil, nil, nil, assistantmodel.WorkflowTriggerLog{
			WorkflowID: workflow.ID, TriggerType: assistantmodel.WorkflowTriggerTypeManual, Status: assistantmodel.WorkflowTriggerLogStatusQueued,
		})
		...
		if failed.Status != assistantmodel.WorkflowTriggerLogStatusFailed || !strings.Contains(failed.Error, "workflow background panic") || failed.FinishedAt == "" {
			t.Fatalf("background panic log = %+v", failed)
		}
	})
}

func TestWorkflowBackgroundPersistsFailureAfterRunningTransitionWriteFails(t *testing.T) {
	runtime, service, _ := newAssistantServiceHarness(t)
	workflow := assistantmodel.WorkflowDefinition{
	...
	prepared, accepted, err := prepareWorkflowInvocation(
		t.Context(), store, workflow, nil, assistantmodel.WorkflowTriggerTypeManual, map[string]any{"symbol": "US.AAPL"}, nil,
	)
	if err != nil || !accepted || prepared.Log.ID == "" || prepared.Log.Status != assistantmodel.WorkflowTriggerLogStatusQueued {
	...
	failed := store.savedLogs[len(store.savedLogs)-1]
	if failed.ID != prepared.Log.ID || failed.Status != assistantmodel.WorkflowTriggerLogStatusFailed || failed.FinishedAt == "" || !strings.Contains(failed.Error, errWorkflowLogWriteInjected.Error()) {
