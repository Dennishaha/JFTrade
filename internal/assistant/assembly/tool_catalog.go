...
func registerADKStrategyResearchTools(registry *jfadk.ToolRegistry, deps ToolDeps) {
	registry.Register(jfadk.ToolDescriptor{Name: "strategy.research_backtest", DisplayName: "策略研究回测", Description: "用临时 Pine Script v6 脚本进行研究回测；会先校验脚本并启动临时回测，但不会保存策略草稿或定义。回测运行和结果会保留供后续查询。", Category: "strategy", Permission: "optimize_strategy", RiskLevel: "low", OutputSummary: "临时回测 runId、状态、脚本 hash、校验摘要和可选结果视图。", RequiredSkills: []string{strategypinespec.ResearchBuiltinSkillName}}, func(ctx context.Context, input map[string]any) (any, error) {
...
func registerADKStrategyWriteTools(registry *jfadk.ToolRegistry, deps ToolDeps) {
	...
	registry.Register(jfadk.ToolDescriptor{Name: "strategy.save_definition", DisplayName: "保存策略定义", Description: "新建或更新 Pine Script v6 策略定义；保存前会强制校验 Pine 并拒绝 JFTrade 暂不支持的执行语义。", Category: "strategy", Permission: "write_strategy", RequiresApprovalIn: []string{jfadk.PermissionModeApproval}, OutputSummary: "保存后的策略定义，以及本次是创建还是更新。", RequiredSkills: []string{strategypinespec.PublishBuiltinSkillName}}, func(_ context.Context, input map[string]any) (any, error) {
...
func registerADKStrategyOptimizationTools(store *jfadk.Store, registry *jfadk.ToolRegistry, deps ToolDeps) {
	registry.Register(jfadk.ToolDescriptor{Name: "strategy.optimize", DisplayName: "策略优化", Description: "为多个候选策略定义创建真实异步回测任务，并返回任务引用。", Category: "strategy", Permission: "optimize_strategy", RequiresApprovalIn: []string{jfadk.PermissionModeApproval}, OutputSummary: "优化任务 ID 与候选回测 Run。", RequiredSkills: []string{strategypinespec.PublishBuiltinSkillName}}, func(_ context.Context, input map[string]any) (any, error) {
