...
func registerJFTradeADKWorkflowManagementTools(store *jfadkruntime.Store, registry *jfadkruntime.ToolRegistry, manager WorkflowToolManager) {
	RegisterWorkflowManagementTools(store, registry, manager)
}

func registerJFTradeProductTools(registry *jfadkruntime.ToolRegistry, deps ToolDeps) {
	RegisterProductTools(registry, ProductToolDeps{
		ProductTool:   deps.ProductTool,
		ExecutionTool: deps.ExecutionTool,
	})
}
...
func registerJFTradeADKStrategyTools(store *jfadkruntime.Store, registry *jfadkruntime.ToolRegistry, deps ToolDeps) {
...
func registerADKStrategyWriteTools(registry *jfadkruntime.ToolRegistry, deps ToolDeps) {
	registry.Register(assistantmodel.ToolDescriptor{Name: "strategy.save_draft", DisplayName: "保存策略草稿", Description: "把 agent 生成的 Pine Script v6 策略脚本保存为策略定义草稿。", Category: "strategy", Permission: "write_strategy", RiskLevel: "low", OutputSummary: "保存后的策略定义。", RequiredSkills: []string{strategypinespec.PublishBuiltinSkillName}}, func(_ context.Context, input map[string]any) (any, error) {
		script := strings.TrimSpace(stringValue(input, "script"))
		if script == "" {
	...
	registry.Register(assistantmodel.ToolDescriptor{Name: "strategy.save_definition", DisplayName: "保存策略定义", Description: "新建或更新 Pine Script v6 策略定义；保存前会强制校验 Pine 并拒绝 JFTrade 暂不支持的执行语义。", Category: "strategy", Permission: "write_strategy", RequiresApprovalIn: []string{assistantmodel.PermissionModeApproval}, OutputSummary: "保存后的策略定义，以及本次是创建还是更新。", RequiredSkills: []string{strategypinespec.PublishBuiltinSkillName}}, func(_ context.Context, input map[string]any) (any, error) {
	...
	registry.Register(assistantmodel.ToolDescriptor{Name: "strategy.update_instance_mode", DisplayName: "修改实例模式", Description: "按 strategy instanceId 修改单个实例的 executionMode，仅允许在实例处于 STOPPED 时执行。", Category: "strategy", Permission: "write_strategy", RequiresApprovalIn: []string{assistantmodel.PermissionModeApproval}, OutputSummary: "更新后的策略实例，以及本次实际修改的字段。", RequiredSkills: []string{strategypinespec.PublishBuiltinSkillName}}, func(_ context.Context, input map[string]any) (any, error) {
...
func registerADKStrategyOptimizationTools(store *jfadkruntime.Store, registry *jfadkruntime.ToolRegistry, deps ToolDeps) {
	registry.Register(assistantmodel.ToolDescriptor{Name: "strategy.optimize", DisplayName: "策略优化", Description: "为多个候选策略定义创建真实异步回测任务，并返回任务引用。", Category: "strategy", Permission: "optimize_strategy", RequiresApprovalIn: []string{assistantmodel.PermissionModeApproval}, OutputSummary: "优化任务 ID 与候选回测 Run。", RequiredSkills: []string{strategypinespec.PublishBuiltinSkillName}}, func(_ context.Context, input map[string]any) (any, error) {
		...
		runRefs := make([]assistantmodel.OptimizationRunRef, 0, len(definitionIDs))
