...
func TestADKWatchlistListReturnsRealDataWithoutImplicitQuoteCalls(t *testing.T) {
	...
	adapter := NewWatchlistToolAdapter(service)

	summaryValue, err := adapter.List(t.Context(), WatchlistListInput{})
	if err != nil {
		t.Fatal(err)
	}
	...
	itemsValue, err := adapter.List(t.Context(), WatchlistListInput{Group: watchlist.DefaultGroupName})
	if err != nil {
		t.Fatal(err)
	}
	...
	quotedValue, err := adapter.List(t.Context(), WatchlistListInput{
...
func TestWatchlistToolAdapterUnavailableAndMissingGroupBoundaries(t *testing.T) {
	if _, err := NewWatchlistToolAdapter(nil).List(t.Context(), WatchlistListInput{}); err == nil {
		t.Fatal("nil watchlist error = nil")
	}
	if _, ok := resolveWatchlistGroup(nil, "missing"); ok {
