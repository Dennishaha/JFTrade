...
const (
	// SourceID is the stable watchlist source identity for Futu OpenD.
...
// NewSourceReader creates the Futu-backed remote watchlist reader.
func NewSourceReader(
	provider func() (broker.WatchlistGroupReader, error),
	enabled func() bool,
	probes ...func(context.Context) error,
) watchlist.WatchlistSourceReader {
	typedProbes := make([]watchlistSourceProbe, 0, len(probes))
	for _, probe := range probes {
		typedProbes = append(typedProbes, probe)
	}
	return newFutuWatchlistReader(provider, enabled, typedProbes...)
}
...
func (r *futuWatchlistReader) Source(ctx context.Context) (watchlist.Source, error) {
	source := watchlist.Source{
		ID: SourceID, Broker: "futu", DisplayName: "Futu OpenD", Status: "ready", UpdatedAt: r.clock().UTC(),
	}
...
func normalizeFutuWatchlistGroups(groups []broker.WatchlistGroup, observedAt time.Time) []watchlist.RemoteGroup {
	...
	for _, group := range groups {
		...
		result = append(result, watchlist.RemoteGroup{
			SourceID: SourceID, RemoteGroupID: futuRemoteGroupID(groupType, name, ordinal, counts[key]),
...
// NewBatchSnapshotSource creates the Futu SecuritySnapshot quote source.
func NewBatchSnapshotSource(provider func() (broker.BatchSnapshotSource, error)) watchlist.BatchSnapshotSource {
	return newFutuWatchlistSnapshotSource(provider)
}
