...
func TestBinaryNameForUsesGoPlatformNames(t *testing.T) {
	tests := []struct {
		...
		{goos: "darwin", goarch: "arm64", want: "marketdata-sidecar-darwin-arm64"},
		{goos: "linux", goarch: "amd64", want: "marketdata-sidecar-linux-amd64"},
		{goos: "windows", goarch: "amd64", want: "marketdata-sidecar-windows-amd64.exe"},
	}
...
func TestSelectFromFSReturnsPlatformOnedirAssetMetadata(t *testing.T) {
	data := []byte("market-data sidecar")
	asset, available, err := selectFromFS(fstest.MapFS{
		"bin/marketdata-sidecar-windows-amd64/marketdata-sidecar-windows-amd64.exe": &fstest.MapFile{Data: data},
		"bin/marketdata-sidecar-windows-amd64/lib/runtime.dll":                      &fstest.MapFile{Data: []byte("runtime")},
	}, "windows", "amd64")
...
func TestSelectFromFSTreatsMissingAndEmptyAssetsAsUnavailable(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "empty", files: fstest.MapFS{
			"bin/marketdata-sidecar-linux-amd64/marketdata-sidecar-linux-amd64": &fstest.MapFile{},
		}},
	} {
...
func TestSelectFromFSRejectsInvalidPlatformBundleShapes(t *testing.T) {
	const (
		root       = "bin/marketdata-sidecar-linux-amd64"
		executable = root + "/marketdata-sidecar-linux-amd64"
	)
	tests := []struct {
		name      string
		files     fs.FS
...
func TestMaterializeAssetUsesPrivateDirectoryAndCleansUp(t *testing.T) {
	asset := Asset{Name: "marketdata-sidecar-linux-amd64", Files: []AssetFile{
		{Path: "marketdata-sidecar-linux-amd64", Data: []byte("#!/bin/sh\nexit 0\n")},
...
func TestMaterializeAssetRejectsDigestChanges(t *testing.T) {
	materialized, available, err := materializeAsset(Asset{
		Name:   "marketdata-sidecar-linux-amd64",
		Files:  []AssetFile{{Path: "marketdata-sidecar-linux-amd64", Data: []byte("sidecar")}},
...
func TestMaterializeAssetRejectsInvalidBundlePath(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "invalid market-data sidecar bundle file path") {
