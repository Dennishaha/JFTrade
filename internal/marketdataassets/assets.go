...
const (
	binDir             = "bin"
	sidecarBaseName    = "marketdata-sidecar"
	tempDirPrefix      = "jftrade-marketdata-sidecar-"
	assetFileMode      = 0o700
...
func materializeAsset(asset Asset) (*MaterializedAsset, bool, error) {
	...
	if !strings.EqualFold(digest, asset.SHA256) {
		return nil, false, errors.New("market-data sidecar bundle SHA256 changed while materializing")
	}
	tempDir, err := os.MkdirTemp("", tempDirPrefix)
	if err != nil {
		return nil, false, fmt.Errorf("create market-data sidecar temp directory: %w", err)
	}
	cleanup := func() {
	...
	if err := os.Chmod(tempDir, assetDirectoryMode); err != nil {
		cleanup()
		return nil, false, fmt.Errorf("restrict market-data sidecar temp directory: %w", err)
	}
	for _, file := range assetFiles {
		...
		if err := os.MkdirAll(filepath.Dir(path), assetDirectoryMode); err != nil {
			cleanup()
			return nil, false, fmt.Errorf("create market-data sidecar bundle directory: %w", err)
		}
		if err := os.WriteFile(path, file.Data, assetFileMode); err != nil {
			cleanup()
			return nil, false, fmt.Errorf("write market-data sidecar bundle file: %w", err)
		}
		if err := os.Chmod(path, assetFileMode); err != nil {
			cleanup()
			return nil, false, fmt.Errorf("restrict market-data sidecar bundle file: %w", err)
		}
	}
	...
	if err != nil {
		cleanup()
		return nil, false, fmt.Errorf("hash market-data sidecar bundle: %w", err)
	}
	if !strings.EqualFold(digest, asset.SHA256) {
		cleanup()
		return nil, false, errors.New("market-data sidecar bundle SHA256 changed while materializing")
...
func readAssetFiles(files fs.FS, root string) ([]AssetFile, error) {
	...
	err := fs.WalkDir(files, root, func(name string, entry fs.DirEntry, err error) error {
		...
		if entry.Type()&fs.ModeSymlink != 0 {
			return fmt.Errorf("market-data sidecar bundle contains unsupported symlink %q", name)
		}
		data, err := fs.ReadFile(files, name)
		if err != nil {
			return fmt.Errorf("read market-data sidecar bundle file %q: %w", name, err)
		}
		relative := strings.TrimPrefix(name, root+"/")
		if relative == name || relative == "." || relative == ".." || strings.HasPrefix(relative, "../") {
			return fmt.Errorf("invalid market-data sidecar bundle file path %q", name)
		}
...
func normalizeAssetFiles(files []AssetFile) ([]AssetFile, error) {
	...
	for index, file := range copyFiles {
		...
		if clean == "." || clean == ".." || strings.HasPrefix(clean, "../") || pathpkg.IsAbs(file.Path) || clean != file.Path {
			return nil, fmt.Errorf("invalid market-data sidecar bundle file path %q", file.Path)
		}
		if index > 0 && copyFiles[index-1].Path == file.Path {
			return nil, fmt.Errorf("duplicate market-data sidecar bundle file path %q", file.Path)
		}
