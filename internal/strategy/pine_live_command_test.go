...
func TestCandleFromKLinePreservesClosedBarWireFields(t *testing.T) {
	start := time.Date(2026, time.July, 26, 10, 0, 0, 0, time.UTC)
	kline := types.KLine{
	...
	candle := CandleFromKLine(kline)
	if candle.OpenTime != start.UnixMilli() || candle.CloseTime != start.Add(time.Minute).UnixMilli() {
		t.Fatalf("candle times = %d..%d", candle.OpenTime, candle.CloseTime)
	}
	if candle.Open != 101 || candle.High != 103 || candle.Low != 99 || candle.Close != 102 || candle.Volume != 500 {
		t.Fatalf("candle values = %#v", candle)
	}
}
...
func TestScopedExitQuantitySurvivesGoExecution(t *testing.T) {
	...
	sizer := newLiveCommandTestSizer("US.AAPL", "USD", types.NewAccount())
