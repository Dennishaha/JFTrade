...
func TestStrategyRuntimeManagerUsesExplicitDependenciesInsteadOfServerOwnership(t *testing.T) {
	...
	depsType := reflect.TypeFor[Dependencies]()
...
func TestStrategyRuntimeManagerStartValidationAndReservationBoundaries(t *testing.T) {
	manager := &Manager{
		runtimes: map[string]*managedRuntime{
			"running": {symbols: map[string]*symbolRuntime{"US.AAPL": {}, "US.MSFT": {}}},
			"second":  {symbols: map[string]*symbolRuntime{"US.AAPL": {}, "HK.00700": {}}},
		},
	}
	...
	err := manager.Start(context.Background(), stratsrv.ManagedInstance{
		ID:      "bad-interval",
...
func TestStrategySymbolRuntimeTradeBucketsAndOrderSignals(t *testing.T) {
	...
	if got := (*symbolRuntime)(nil).context(); got == nil {
...
func TestStrategyRuntimeAccountAndFormattingBoundaries(t *testing.T) {
	...
	if got := strategyRuntimeDisplayName(stratsrv.ManagedInstance{ID: "inst-id"}, &symbolRuntime{name: " runner-name "}); got != "runner-name" {
...
func TestStrategyRuntimeSymbolMarketAndStartErrorBoundaries(t *testing.T) {
	...
	if code, label := strategyRuntimeStartError(errors.New("broker gateway down")); code != 502 ||
		label != "STRATEGY_RUNTIME_START_FAILED" {
