...
func (r *symbolRuntime) syncClosedKLinesLoop() {
	if r == nil || r.closedKLineSyncInterval <= 0 {
		return
	}
	ticker := time.NewTicker(r.closedKLineSyncInterval)
	defer ticker.Stop()
	ctx := r.ctx
	if ctx == nil {
...
func (r *symbolRuntime) handleTrade(trade bbgotypes.Trade) {
...
func (r *symbolRuntime) recordClosedKLineState(closed bbgotypes.KLine) bool {
...
func (r *symbolRuntime) emitClosedKLine(closed bbgotypes.KLine) {
...
func (r *symbolRuntime) context() context.Context {
	if r != nil && r.ctx != nil {
		return r.ctx
	}
...
func (r *symbolRuntime) handleRuntimeError(err error) {
	if err == nil {
		return
	}
	if r.onError != nil {
...
func (r *symbolRuntime) brokerPositionsSnapshot() []broker.PositionSnapshot {
	if r == nil {
		return nil
	}
	r.accountMu.RLock()
...
func (r *symbolRuntime) brokerAccountSnapshot() *bbgotypes.Account {
...
func (r *symbolRuntime) currentPrice() float64 {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *symbolRuntime) setCurrentBucket(bucket *bbgotypes.KLine) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *symbolRuntime) setLastClosedPrice(price float64) {
	r.mu.Lock()
