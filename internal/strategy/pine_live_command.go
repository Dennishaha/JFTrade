...
// WorkerOrderCommand is the broker-neutral strategy command emitted by the
// Pine worker. It deliberately contains no backtest or broker implementation
// types, so live strategy control can own validation and execution policy.
...
// CommandsFromOrderIntents validates Pine worker output and expands composite
// exits into broker-neutral commands for the live strategy control plane.
...
// CommandFromOrderIntent converts one simple worker intent. Composite OCO
// exits are expanded by CommandsFromOrderIntents.
...
// CandleFromKLine converts one closed market bar into the Pine worker wire shape.
