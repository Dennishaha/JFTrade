...
// LiveNotificationFromResponse converts an OpenD system notification into the
// broker-neutral event consumed by the live and assistant domains.
func LiveNotificationFromResponse(response *notifypb.Response) *live.Notification {
	...
	case notifypb.NotifyType_NotifyType_GtwEvent:
		return GatewayEventNotification(s2c.GetEvent())
	...
	case notifypb.NotifyType_NotifyType_ConnStatus:
		return ConnectionStatusNotification(s2c.GetConnectStatus())
	case notifypb.NotifyType_NotifyType_QotRight:
		return QuoteRightNotification(s2c.GetQotRight())
...
func ConnectionStatusNotification(status *notifypb.ConnectStatus) *live.Notification {
	if status == nil {
		return nil
	}
...
func GatewayEventNotification(event *notifypb.GtwEvent) *live.Notification {
	if event == nil {
		return nil
	}
	eventType := notifypb.GtwEventType(event.GetEventType())
	note := baseFutuNotification("broker.event")
	note.Level = GatewayEventLevel(eventType)
	note.Title = GatewayEventTitle(eventType)
	label := GatewayEventLabel(eventType)
...
func QuoteRightNotification(right *notifypb.QotRight) *live.Notification {
	if right == nil {
		return nil
	}
...
func GatewayEventLevel(value notifypb.GtwEventType) string {
	switch value {
...
func GatewayEventTitle(value notifypb.GtwEventType) string {
	switch value {
...
func GatewayEventLabel(value notifypb.GtwEventType) string {
	switch value {
	case notifypb.GtwEventType_GtwEventType_None:
		return "无异常"
...
func formatQotRightSummary(right *notifypb.QotRight) string {
	...
	appendRight := func(label string, value *int32) {
		...
		parts = append(parts, fmt.Sprintf("%s %s", label, QuoteRightLabel(*value)))
	}
...
func QuoteRightLabel(value int32) string {
