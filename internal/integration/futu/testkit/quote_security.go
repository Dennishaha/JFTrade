...
func marketDataSecuritySnapshotFixture(security *qotcommonpb.Security, quoteAt time.Time) *qotgetsecuritysnapshotpb.Snapshot {
	...
	switch code {
	case "21164":
		return warrantSnapshotFixture(security, quoteAt)
	case "AAPL250117C00200000":
		return optionSnapshotFixture(security, quoteAt)
	case "HSIMAIN":
		return futureSnapshotFixture(security, quoteAt)
	case "SPY":
		return trustSnapshotFixture(security, quoteAt)
	case "HSI":
		return indexSnapshotFixture(security, quoteAt)
	case "TECH":
		return plateSnapshotFixture(security, quoteAt)
	default:
		return equitySnapshotFixture(security, quoteAt)
	}
}

func warrantSnapshotFixture(security *qotcommonpb.Security, quoteAt time.Time) *qotgetsecuritysnapshotpb.Snapshot {
	return &qotgetsecuritysnapshotpb.Snapshot{
		...
		WarrantExData: &qotgetsecuritysnapshotpb.WarrantSnapshotExData{
			ConversionRate: new(float64(10000)), WarrantType: new(int32(qotcommonpb.WarrantType_WarrantType_Bull)),
			StrikePrice: new(float64(320)), MaturityTime: new("2026-12-30"), EndTradeTime: new("2026-12-29"),
			Owner:         marketDataProtoSecurity(qotcommonpb.QotMarket_QotMarket_HK_Security, "00700"),
			RecoveryPrice: new(float64(300)), StreetVolumn: new(int64(32000000)), IssueVolumn: new(int64(64000000)),
			StreetRate: new(float64(50)), Delta: new(0.48), ImpliedVolatility: new(28.5), Premium: new(12.6),
			MaturityTimestamp: new(float64(1798588800)), EndTradeTimestamp: new(float64(1798502400)),
			Leverage: new(8.2), Ipop: new(4.7), BreakEvenPoint: new(334.2), ConversionPrice: new(0.032),
			PriceRecoveryRatio: new(6.8), Score: new(float64(85)), IssuerCode: new("SG"),
		},
	}
}

func optionSnapshotFixture(security *qotcommonpb.Security, quoteAt time.Time) *qotgetsecuritysnapshotpb.Snapshot {
	return &qotgetsecuritysnapshotpb.Snapshot{
		...
		OptionExData: &qotgetsecuritysnapshotpb.OptionSnapshotExData{
			...
			StrikeTime: new("2025-01-17"), StrikePrice: new(float64(200)), ContractSize: new(int32(100)),
			ContractSizeFloat: new(float64(100)), OpenInterest: new(int32(14280)), ImpliedVolatility: new(24.2),
			Premium: new(3.4), Delta: new(0.61), Gamma: new(0.02), Vega: new(0.11), Theta: new(-0.08), Rho: new(0.04),
			StrikeTimestamp: new(float64(1737072000)), ExpiryDateDistance: new(int32(45)),
			ContractNominalValue: new(float64(20000)), OwnerLotMultiplier: new(float64(1)), ContractMultiplier: new(float64(100)),
		},
	}
}

func futureSnapshotFixture(security *qotcommonpb.Security, quoteAt time.Time) *qotgetsecuritysnapshotpb.Snapshot {
	return &qotgetsecuritysnapshotpb.Snapshot{
		...
		FutureExData: &qotgetsecuritysnapshotpb.FutureSnapshotExData{
			LastSettlePrice: new(float64(18390)), Position: new(int32(182233)), PositionChange: new(int32(4201)),
			LastTradeTime: new("2026-06-29"), LastTradeTimestamp: new(float64(1782691200)), IsMainContract: new(true),
		},
	}
}

func trustSnapshotFixture(security *qotcommonpb.Security, quoteAt time.Time) *qotgetsecuritysnapshotpb.Snapshot {
	return &qotgetsecuritysnapshotpb.Snapshot{
		...
		TrustExData: &qotgetsecuritysnapshotpb.TrustSnapshotExData{
			DividendYield: new(1.3), Aum: new(float64(580000000000)), OutstandingUnits: new(int64(985000000)),
			NetAssetValue: new(589.8), Premium: new(0.14), AssetClass: new(int32(qotcommonpb.AssetClass_AssetClass_Stock)),
		},
	}
}

func indexSnapshotFixture(security *qotcommonpb.Security, quoteAt time.Time) *qotgetsecuritysnapshotpb.Snapshot {
	return &qotgetsecuritysnapshotpb.Snapshot{
		...
		IndexExData: &qotgetsecuritysnapshotpb.IndexSnapshotExData{RaiseCount: new(int32(58)), FallCount: new(int32(21)), EqualCount: new(int32(3))},
	}
}

func plateSnapshotFixture(security *qotcommonpb.Security, quoteAt time.Time) *qotgetsecuritysnapshotpb.Snapshot {
	return &qotgetsecuritysnapshotpb.Snapshot{
		...
		PlateExData: &qotgetsecuritysnapshotpb.PlateSnapshotExData{RaiseCount: new(int32(42)), FallCount: new(int32(17)), EqualCount: new(int32(5))},
	}
}

func equitySnapshotFixture(security *qotcommonpb.Security, quoteAt time.Time) *qotgetsecuritysnapshotpb.Snapshot {
	return &qotgetsecuritysnapshotpb.Snapshot{
		...
		EquityExData: &qotgetsecuritysnapshotpb.EquitySnapshotExData{
			IssuedShares: new(int64(9600000000)), IssuedMarketVal: new(float64(3085440000000)),
			NetAsset: new(float64(950000000000)), NetProfit: new(float64(185000000000)), EarningsPershare: new(19.2),
			OutstandingShares: new(int64(9300000000)), OutstandingMarketVal: new(float64(2989020000000)),
			NetAssetPershare: new(98.3), EyRate: new(6.0), PeRate: new(16.7), PbRate: new(3.2), PeTTMRate: new(17.1),
			DividendTTM: new(3.4), DividendRatioTTM: new(1.1),
		},
	}
}
