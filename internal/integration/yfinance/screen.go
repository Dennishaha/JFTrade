...
// Screen executes an embedded-catalog stock screen. Yahoo Finance screener
// data only covers US equities; other markets are rejected Go-side as
// ErrUnsupported before any sidecar call.
func (p *Provider) Screen(
	ctx context.Context,
	req marketdata.ScreenRequest,
) (marketdata.ScreenResponse, error) {
	canonical, err := canonicalMarket(req.Market)
	if err != nil {
		return marketdata.ScreenResponse{}, err
	}
	if canonical != yfinanceDefaultMarket {
		return marketdata.ScreenResponse{}, fmt.Errorf("%w: stock screen market %q", ErrUnsupported, req.Market)
	}
	response, err := p.client.screen(ctx, toRemoteScreenRequest(req, canonical))
	if err != nil {
		return marketdata.ScreenResponse{}, classifyScreenError(err)
	}
	return convertScreen(response, canonical)
}
...
func convertScreen(
	...
	for index, entry := range response.Entries {
		...
		symbol := strings.ToUpper(instrumentID)
		if entry.Symbol != nil {
...
func screenSource(source, market string) string {
	...
	return "yfinance-screen-" + strings.ToLower(market)
}
