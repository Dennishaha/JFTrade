...
func TestChatStreamHubKeepsEventAndTimelineContracts(t *testing.T) {
	t.Run("unserializable transient tool output does not discard the stream event", func(t *testing.T) {
		event := adkChatStreamEvent{Type: "run", Run: &assistantmodel.Run{
	...
	t.Run("timeline-only deltas are consumed without emitting narrative duplicates", func(t *testing.T) {
		...
		timeline := assistantmodel.TimelineEntry{RunID: "coverage98-timeline", Kind: assistantmodel.TimelineKindAssistantMessage, Text: "already projected"}
		if err := execution.handleDelta(assistantmodel.ChatDelta{Timeline: &timeline}); err != nil {
...
func TestChatStreamExecutionReusesKnownContextAndRecoversTerminalRun(t *testing.T) {
	runtime, _ := newAssistantTestRouter(t)
	ctx := t.Context()
	agent, err := runtime.Store().SaveAgent(ctx, assistantmodel.AgentWriteRequest{
		ID: "coverage98-stream-agent", Name: "Coverage Stream Agent", Status: assistantmodel.AgentStatusEnabled,
	})
	if err != nil {
	...
	t.Run("existing context does not get re-fetched after preview session", func(t *testing.T) {
		...
		execution := newADKChatStreamExecution(handler, record, assistantmodel.ChatRequest{SessionID: session.ID, Message: "continue"})
	...
	t.Run("persisted terminal run is published as final recovery instead of a second error", func(t *testing.T) {
		run := assistantmodel.Run{
			ID: "coverage98-terminal-run", SessionID: session.ID, AgentID: agent.ID, Status: assistantmodel.RunStatusCompleted,
		...
		execution := newADKChatStreamExecution(handler, record, assistantmodel.ChatRequest{})
