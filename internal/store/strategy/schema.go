...
func initializeStrategyDatabase(db sqliteschema.Database, path string) error {
	return sqliteschema.InitializeCurrent(context.Background(), db, path, sqliteschema.DatabaseStrategy)
}
...
func strategyDatabaseStatements() []string {
	return []string{
		...
		strings.Join([]string{`CREATE TABLE ` + strategyDesignDefinitionVersionTable + ` (`,
			`definition_id TEXT NOT NULL, version TEXT NOT NULL, name TEXT NOT NULL DEFAULT '',`,
			`description TEXT NOT NULL DEFAULT '', runtime TEXT NOT NULL DEFAULT '', source_format TEXT NOT NULL DEFAULT '',`,
			`symbol TEXT NOT NULL DEFAULT '', interval TEXT NOT NULL DEFAULT '', script TEXT NOT NULL DEFAULT '',`,
			`visual_model_json TEXT NOT NULL DEFAULT '', created_at TEXT NOT NULL DEFAULT '',`,
			`updated_at TEXT NOT NULL DEFAULT '', saved_at TEXT NOT NULL DEFAULT '',`,
			`PRIMARY KEY (definition_id, version),`,
			`FOREIGN KEY (definition_id) REFERENCES ` + strategyDesignDefinitionTable + `(id) ON DELETE CASCADE)`}, " "),
		...
		`CREATE INDEX idx_strategy_definition_versions_saved_at ON ` + strategyDesignDefinitionVersionTable + ` (definition_id, saved_at DESC, version DESC)`,
		`CREATE TRIGGER trg_strategy_definition_versions_immutable
			BEFORE UPDATE ON ` + strategyDesignDefinitionVersionTable + `
			BEGIN
...
func validateStrategyDatabase(ctx context.Context, db sqliteschema.Database) error {
	for _, schema := range []struct {
		table   string
		columns []string
	}{
		{strategyRuntimeLogTable, expectedStrategyRuntimeLogColumns()},
		{strategyRuntimeAuditTable, expectedStrategyRuntimeAuditColumns()},
		{strategyRuntimeObservationTable, expectedStrategyRuntimeObservationColumns()},
		...
		{strategyDesignDefinitionVersionTable, []string{
			"definition_id:TEXT:1", "version:TEXT:2", "name:TEXT:0", "description:TEXT:0", "runtime:TEXT:0",
			"source_format:TEXT:0", "symbol:TEXT:0", "interval:TEXT:0", "script:TEXT:0",
			"visual_model_json:TEXT:0", "created_at:TEXT:0", "updated_at:TEXT:0", "saved_at:TEXT:0",
		}},
	} {
...
func expectedStrategyRuntimeLogColumns() []string {
	return []string{
		"id:INTEGER:1", "instance_id:TEXT:0", "at_ms:INTEGER:0",
		"raw:TEXT:0", "level:TEXT:0", "source:TEXT:0",
	}
}

func expectedStrategyRuntimeAuditColumns() []string {
	return []string{
		"id:INTEGER:1", "instance_id:TEXT:0", "kind:TEXT:0",
		"detail:TEXT:0", "at_ms:INTEGER:0",
	}
}

func expectedStrategyRuntimeObservationColumns() []string {
	return []string{
		"instance_id:TEXT:1", "actual_status_snapshot:TEXT:0", "active_symbols_json:TEXT:0",
		"last_closed_kline_at_ms:INTEGER:0", "last_signal_at_ms:INTEGER:0",
		"last_order_at_ms:INTEGER:0", "last_error_at_ms:INTEGER:0",
		"last_error:TEXT:0", "updated_at_ms:INTEGER:0",
	}
}
