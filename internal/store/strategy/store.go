...
const (
	...
	RuntimePinePlan                      = stratsrv.RuntimePinePlan
	DefaultVersion                       = stratsrv.DefaultVersion
	strategyRuntimePinePlan              = RuntimePinePlan
	defaultStrategyVersion               = DefaultVersion
)

var errUnsupportedLegacyStrategyDefinition = stratsrv.ErrUnsupportedLegacyDefinition
...
// New opens a strategy design resource behind its consumer-owned interfaces.
func New(path string) (Resource, error) {
	return openStore(path)
}

func openStore(path string) (*Store, error) {
	store := &Store{path: path, dbPath: DeriveDBPath(path)}
...
// NewUnavailable returns a non-persisting placeholder after bootstrap records
// an incompatible or unavailable strategy database.
func NewUnavailable(path string) Resource {
	return newUnavailableStore(path)
}

func newUnavailableStore(path string) *Store {
	return &Store{path: path, dbPath: DeriveDBPath(path)}
}
...
func (s *Store) ListDefinitionVersions(definitionID string) ([]stratsrv.DefinitionVersionSummary, bool, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Store) GetDefinitionVersion(definitionID string, version string) (stratsrv.DefinitionVersion, bool, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	definitionID = strings.TrimSpace(definitionID)
...
func (s *Store) saveDefinitionToDBLocked(normalized stratsrv.Definition) (stratsrv.Definition, error) {
...
func (s *Store) DeleteDefinition(id string) (stratsrv.Definition, error) {
	...
	if !ok {
		return stratsrv.Definition{}, stratsrv.NotFoundError("strategy resource not found")
	}
	definition, err := strategyDesignDefinitionFromRow(row)
	if err != nil {
		return stratsrv.Definition{}, err
...
func (s *Store) definitionRowLocked(id string, includeDeleted bool) (strategyDesignDefinitionRow, bool, error) {
...
func (s *Store) upsertDefinitionLocked(definition stratsrv.Definition, deletedAt *string) error {
