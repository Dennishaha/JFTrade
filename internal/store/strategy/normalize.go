...
func defaultStrategyDesignPine(name string) string {
	return stratsrv.DefaultPine(name)
}

// DefaultPine returns the canonical starter Pine v6 strategy source.
func DefaultPine(name string) string { return stratsrv.DefaultPine(name) }
...
// NormalizeVisualModel applies persisted visual-model defaults and rejects
// builder blocks that no longer have a Pine v6 representation.
func NormalizeVisualModel(model *stratsrv.VisualModel) (*stratsrv.VisualModel, error) {
