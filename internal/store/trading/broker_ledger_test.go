...
func TestBrokerFeeNilAndMissingOrderBoundaries(t *testing.T) {
	store := newExecutionOrderStore()
	if _, event, changed := store.ApplyBrokerFee("futu", broker.OrderFeeSnapshot{}); event != nil || changed {
		t.Fatalf("empty fee result = event=%+v changed=%v", event, changed)
	}
	amount := 1.5
	if _, event, changed := store.ApplyBrokerFee("futu", broker.OrderFeeSnapshot{
