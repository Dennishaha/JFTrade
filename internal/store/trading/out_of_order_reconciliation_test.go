...
func seedOutOfOrderPlacedOrder(store *Store, brokerOrderID string) trdsrv.ExecutionOrder {
	price := 100.0
...
func TestExecutionOrderStoreIgnoresOutOfOrderRegressionPushAfterTerminalState(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			advanced, _, changed := store.ApplyBrokerOrder("futu",
			...
			regressed, event, changed := store.ApplyBrokerOrder("futu",
...
func TestExecutionOrderStoreAppliesFillProgressFromOlderSnapshot(t *testing.T) {
	...
	// 先在 01:05 确认部分成交 2 股。
	partial, _, changed := store.ApplyBrokerOrder("futu",
	...
	progressed, event, changed := store.ApplyBrokerOrder("futu",
...
func TestExecutionOrderStoreKeepsUpdatedAtMonotonicForOlderFill(t *testing.T) {
	...
	status := "FILLED_PART"
	fillPrice := 101.0
	filled, event, changed := store.ApplyBrokerFill("futu", broker.OrderFillSnapshot{
		AccountID: "SIM-1", TradingEnvironment: "SIMULATE", Market: "US",
...
func TestExecutionOrderStoreResolvesCancelRequestRaceAgainstBrokerPush(t *testing.T) {
	t.Run("fill wins over in-flight cancel", func(t *testing.T) {
		...
		cancelRequested, _, changed := store.ApplyBrokerOrder("futu",
		...
		filled, event, changed := store.ApplyBrokerOrder("futu",
	...
	t.Run("cancel confirmed when no fill arrives", func(t *testing.T) {
		...
		if _, _, changed := store.ApplyBrokerOrder("futu",
		...
		cancelled, event, changed := store.ApplyBrokerOrder("futu",
...
func TestExecutionOrderStoreDuplicateTerminalPushIsNoOp(t *testing.T) {
	...
	filled, _, changed := store.ApplyBrokerOrder("futu",
	...
	_, duplicateEvent, duplicateChanged := store.ApplyBrokerOrder("futu",
		outOfOrderPushSnapshot("ooo-5", "FILLED_ALL", 10, filledAt),
		"BROKER_PUSH_DISCOVERED", "BROKER_PUSH_ORDER", "broker", "broker.push")
