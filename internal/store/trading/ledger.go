...
func (s *Store) listOrders() trdsrv.ExecutionOrders {
	return s.listOrdersFiltered(trdsrv.ExecutionOrderFilter{})
}

func (s *Store) listOrdersFiltered(filter trdsrv.ExecutionOrderFilter) trdsrv.ExecutionOrders {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Store) orderEvents(internalOrderID string) trdsrv.ExecutionOrderEvents {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *Store) findPlacedOrderLocked(input trdsrv.ExecutionPlacedOrderRecord) string {
...
func (s *Store) prepareSubmission(input trdsrv.ExecutionPlacedOrderRecord) (trdsrv.ExecutionOrder, bool, error) {
	...
	if s.persistence != nil {
		s.databaseMu.Lock()
		err := s.persistence.persistOrder(prepared)
		s.databaseMu.Unlock()
		if err != nil {
...
func (s *Store) markSubmissionUnknown(internalOrderID string, submitErr error) trdsrv.ExecutionOrder {
	...
	if s.persistence != nil {
		s.databaseMu.Lock()
		err := s.persistence.persistOrder(cloned)
		s.databaseMu.Unlock()
		if err != nil {
...
func (s *Store) mergePlacedOrderLocked(internalOrderID string, input trdsrv.ExecutionPlacedOrderRecord, submittedAt string, createdAt string) trdsrv.ExecutionOrder {
...
func (s *Store) appendEventLocked(internalOrderID string, previousStatus *string, nextStatus string, eventType string, payload any, createdAt string) trdsrv.ExecutionOrderEvent {
	s.nextEventSeq++
...
func (s *Store) persistOrderLocked(order trdsrv.ExecutionOrder) {
	s.enqueuePersistence(executionPersistenceItem{kind: "order", order: cloneExecutionOrderSummary(order)})
}

func (s *Store) persistEventLocked(event trdsrv.ExecutionOrderEvent) {
	s.enqueuePersistence(executionPersistenceItem{kind: "event", event: cloneExecutionOrderEvent(event)})
}

func (s *Store) persistSeenFillKeyLocked(fillKey string, createdAt string) {
	s.enqueuePersistence(executionPersistenceItem{kind: "fill", fillKey: strings.TrimSpace(fillKey), createdAt: createdAt})
}

func (s *Store) persistSequenceLocked(name string, value uint64) {
	s.enqueuePersistence(executionPersistenceItem{kind: "sequence", seqName: name, seqValue: value})
}

func (s *Store) findInternalOrderIDLocked(brokerID string, accountID string, tradingEnvironment string, market string, brokerOrderID string, brokerOrderIDEx *string) string {
...
func (s *Store) findClientOrderIDLocked(brokerID, tradingEnvironment, accountID, clientOrderID string) string {
...
func (s *Store) linkBrokerOrderLocked(order trdsrv.ExecutionOrder) {
...
func (s *Store) runPersistenceWorker(queue <-chan executionPersistenceItem) {
...
func (s *Store) enqueuePersistence(item executionPersistenceItem) {
	if s == nil || s.persistence == nil {
		return
	}
	s.persistenceMu.Lock()
...
func (s *Store) writePersistenceItem(item executionPersistenceItem) {
	if s == nil || s.persistence == nil {
		return
	}
	s.databaseMu.Lock()
