...
func (s *Store) upsertBrokerOrderWithSource(brokerID string, snapshot broker.OrderSnapshot, discoveredEventType string, updatedEventType string, source string, sourceDetail string) (trdsrv.ExecutionOrder, *trdsrv.ExecutionOrderEvent, bool) {
...
func (s *Store) recordBrokerOrderFill(brokerID string, fill broker.OrderFillSnapshot) (trdsrv.ExecutionOrder, *trdsrv.ExecutionOrderEvent, bool) {
	now := time.Now().UTC().Format(time.RFC3339Nano)
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Store) brokerSnapshotCoveredFillQuantityLocked(internalOrderID string, fill broker.OrderFillSnapshot) float64 {
...
func (s *Store) recordBrokerOrderFee(
	brokerID string,
...
func (s *Store) allocateInternalOrderIDLocked() string {
	s.nextOrderSeq++
	s.persistSequenceLocked("orders", s.nextOrderSeq)
	return fmt.Sprintf("exec-%06d", s.nextOrderSeq)
}

func (s *Store) persistBrokerOrderLocked(summary trdsrv.ExecutionOrder, previousStatus *string, eventType string, payload any, createdAt string) (trdsrv.ExecutionOrder, *trdsrv.ExecutionOrderEvent) {
...
func (s *Store) registerFillKeyLocked(fillKey string, now string) bool {
	if fillKey == "" {
		return false
	}
...
func (s *Store) brokerOrderSummaryFromSnapshot(internalOrderID string, brokerID string, snapshot broker.OrderSnapshot, now string, source string, sourceDetail string) trdsrv.ExecutionOrder {
