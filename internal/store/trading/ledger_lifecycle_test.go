...
func TestExecutionStorePersistsParentBrokerFeesWithoutInventingLegAllocation(t *testing.T) {
	...
	updated, event, changed := store.ApplyBrokerFee("coverage", broker.OrderFeeSnapshot{
	...
	if _, duplicateEvent, duplicateChanged := store.ApplyBrokerFee("coverage", broker.OrderFeeSnapshot{
		AccountID: "account", TradingEnvironment: "SIMULATE", Market: "US",
