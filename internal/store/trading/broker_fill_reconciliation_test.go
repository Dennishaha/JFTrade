...
func TestExecutionOrderStoreReconcilesFillBeforeOrderSnapshot(t *testing.T) {
	...
	synced, syncEvent, changed := store.ApplyBrokerOrder("futu", broker.OrderSnapshot{
		AccountID:          "SIM-OUT-OF-ORDER",
		TradingEnvironment: "SIMULATE",
		Market:             "US",
		BrokerOrderID:      "order-before-sync",
...
func TestExecutionOrderStoreDoesNotDoubleCountSnapshotCoveredFill(t *testing.T) {
	...
	snapshot, _, changed := store.ApplyBrokerOrder("futu", broker.OrderSnapshot{
	...
	delayedPrice := 100.0
	delayed, delayedEvent, changed := store.ApplyBrokerFill("futu", broker.OrderFillSnapshot{
		AccountID: "SIM-1", TradingEnvironment: "SIMULATE", Market: "US",
	...
	newPrice := 102.0
	updated, _, changed := store.ApplyBrokerFill("futu", broker.OrderFillSnapshot{
		AccountID: "SIM-1", TradingEnvironment: "SIMULATE", Market: "US",
...
func TestExecutionOrderStoreRejectsStaleBrokerSnapshotRegression(t *testing.T) {
	...
	filled, _, changed := store.ApplyBrokerOrder("futu", broker.OrderSnapshot{
		AccountID: "SIM-1", TradingEnvironment: "SIMULATE", Market: "US",
	...
	zeroFilled := 0.0
	_, staleEvent, changed := store.ApplyBrokerOrder("futu", broker.OrderSnapshot{
		AccountID: "SIM-1", TradingEnvironment: "SIMULATE", Market: "US",
		BrokerOrderID: "stale-1", Symbol: "US.AAPL", Side: "BUY", OrderType: "LIMIT",
