...
func TestSettingsNormalizationHandlesFallbacksAndBoundaries(t *testing.T) {
...
func TestSettingsInterfaceAndAccountNormalization(t *testing.T) {
...
func TestSettingsStorePersistsValuesAndManagesAccountLifecycle(t *testing.T) {
	path := filepath.Join(t.TempDir(), "settings.json")
	store, err := New(path)
	if err != nil {
		t.Fatalf("New: %v", err)
...
func TestSettingsFileReportsInterfaceAndLoadFailures(t *testing.T) {
