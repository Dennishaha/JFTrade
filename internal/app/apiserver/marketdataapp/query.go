...
func DecodeSnapshotQuery(values map[string][]string) (SnapshotQuery, error) {
	...
	if raw, ok := firstQueryValue(values, "refresh"); ok && raw != "" {
		if err := query.Refresh.UnmarshalText([]byte(raw)); err != nil {
			return SnapshotQuery{}, fmt.Errorf("invalid refresh query: %w", err)
		}
	}
	return query, nil
}

func DecodeCandlesQuery(values map[string][]string) (CandlesQuery, error) {
	var query CandlesQuery
	if raw, ok := values["sessions"]; ok {
		sessions, err := mdsrv.ParseCandleSessions(raw)
		if err != nil {
			return CandlesQuery{}, err
		}
...
func KLineQueryWindow(query CandlesQuery, periodDuration time.Duration, limit int) (time.Time, time.Time) {
	endAt := time.Now()
