...
var _ middleware.OriginChecker = (*Auth)(nil)
var _ middleware.Authenticator = (*Auth)(nil)
var _ middleware.CSRFValidator = (*Auth)(nil)
...
// Auth owns browser password sessions. The Wails WebView uses a separate,
...
func NewAuth(settings jfsettings.SecuritySettings) *Auth {
...
func (a *Auth) Configure(settings jfsettings.SecuritySettings) {
	if a == nil {
		return
	}
...
func (a *Auth) CurrentAccessContext() context.Context {
	if a == nil {
...
func (a *Auth) ConfigureOrigins(values ...string) {
	if a == nil {
		return
	}
	a.mu.Lock()
...
func (a *Auth) originAllowedForRequest(r *http.Request, value string) bool {
	if a == nil {
		return false
	}
...
func (a *Auth) IsOriginAllowed(r *http.Request, value string) bool {
...
func (a *Auth) BrowserAccessAllowed(r *http.Request) bool {
	if a == nil {
		return false
	}
	a.mu.Lock()
...
func (a *Auth) WebAccessEnabled() bool {
	if a == nil {
		return false
	}
	a.mu.Lock()
...
// SetEnforceAccess controls the explicit desktop/test bypass used by the
// sidecar composition root. Browser authentication remains disabled when the
// web access setting is off, but development desktop mode may opt into the
// trusted-host capability without a password session.
func (a *Auth) SetEnforceAccess(enforce bool) {
	if a == nil {
		return
	}
	a.mu.Lock()
	a.enforceAccess = enforce
	a.mu.Unlock()
}

func (a *Auth) EnforceAccess() bool {
	if a == nil {
		return false
	}
	a.mu.Lock()
	defer a.mu.Unlock()
	return a.enforceAccess
}

func (a *Auth) authenticate(r *http.Request) (webSession, bool, bool) {
	if middleware.IsRequestTrustedHost(r) {
...
func (a *Auth) Authenticate(r *http.Request) (string, bool, bool) {
	session, ok, trusted := a.authenticate(r)
	return session.CSRF, ok, trusted
}

// ValidateCSRF implements middleware.CSRFValidator.
func (a *Auth) ValidateCSRF(r *http.Request, session string) bool {
...
func (a *Auth) loginConfig() webLoginConfig {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *Auth) checkPasswordForLogin(ctx context.Context, encoded string, password string, verify func(string, string) (bool, error)) (bool, error) {
	select {
...
func (a *Auth) createSession(generation uint64, passwordHash string) (string, webSession, error) {
...
func (a *Auth) loginRateLimited(remote string) (time.Duration, bool) {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *Auth) recordLoginFailure(remote string) {
	a.mu.Lock()
	defer a.mu.Unlock()
	now := a.now()
...
func (a *Auth) pruneLoginAttemptsLocked(now time.Time) {
...
func (a *Auth) evictOldestLoginAttemptLocked() {
	oldestKey := ""
...
func (a *Auth) pruneSessionsLocked(now time.Time) {
...
func (a *Auth) evictOldestSessionLocked() {
	oldestID := ""
	var oldest time.Time
	for id, session := range a.sessions {
...
func (a *Auth) clearLoginFailures(remote string) {
	a.mu.Lock()
	delete(a.attempts, remote)
	a.mu.Unlock()
}
