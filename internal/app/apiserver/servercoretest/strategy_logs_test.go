...
func TestStrategiesEndpointReturnsList(t *testing.T) {
	...
	instanceID := seedStrategyCatalogInstance(t, catalog, stratsrv.ManagedInstance{
		ID:       "instance-1",
		...
		Status:    strategycatalog.StatusRunning,
		CreatedAt: "2026-05-22T00:00:00Z",
	})
	if err := catalog.AppendRuntimeEvent(instanceID, "started", "started", "mean-revert"); err != nil {
...
func TestStrategiesEndpointIncludesPersistedRuntimeLogTail(t *testing.T) {
	...
	instanceID := seedStrategyCatalogInstance(t, catalog, stratsrv.ManagedInstance{
		...
		Params:    map[string]any{"window": 20},
		Status:    strategycatalog.StatusStopped,
		CreatedAt: "2026-05-22T00:00:00Z",
	})
	if err := catalog.AppendRuntimeEvent(instanceID, "runtime error US.AAPL: boom", "runtime_error", "boom"); err != nil {
		t.Fatalf("appendStrategyRuntimeEvent: %v", err)
	}
	if err := catalog.Close(); err != nil {
		t.Fatalf("close strategy catalog: %v", err)
...
func TestStrategyLogsAndAuditEndpointsSupportPaginationAndFilters(t *testing.T) {
	...
	initialLogs, ok := catalog.GetLogs(instanceID, stratsrv.LogQuery{})
	if !ok {
	...
	for _, event := range []struct {
		...
		if err := catalog.AppendRuntimeEvent(instanceID, event.message, event.kind, event.detail); err != nil {
