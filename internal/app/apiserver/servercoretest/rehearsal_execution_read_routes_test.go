...
type executionReadRehearsalTarget struct{ endpoint string }

func (t executionReadRehearsalTarget) Endpoint() string { return t.endpoint }

func (executionReadRehearsalTarget) BearerToken() string { return strings.Repeat("e", 64) }

func (executionReadRehearsalTarget) Profile() string { return rustrehearsal.ReadOnlyProfile }

func (executionReadRehearsalTarget) Capabilities() []string {
	return append([]string(nil), executionReadRouteOperations...)
}

type executionReadWireSnapshot struct {
	status  int
	body    string
	headers map[string][]string
}

func TestExecutionReadRehearsalPreservesWireAndRequiresRestartForGoRollback(t *testing.T) {
	...
	snapshots := map[string]executionReadWireSnapshot{}
	rust := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		if request.Header.Get("Authorization") != "Bearer "+strings.Repeat("e", 64) ||
		...
		if request.Method != http.MethodGet || request.ContentLength > 0 {
			t.Errorf("execution read rehearsal forwarded a non-read request: method=%s contentLength=%d", request.Method, request.ContentLength)
		}
		...
		if err != nil {
			t.Errorf("call Go owner for execution wire comparison: %v", err)
			return
		}
		...
		snapshot := executionReadWireSnapshot{
			status:  ownerResponse.StatusCode,
			body:    string(body),
			headers: executionReadHeaderSnapshot(ownerResponse.Header),
		}
		snapshotsMu.Lock()
	...
	proxyOwner := servercore.NewSidecarHandlerWithOptions(store, servercore.SidecarOptions{
		DesktopMode:           true,
		RehearsalTarget:       executionReadRehearsalTarget{endpoint: rust.URL},
		RehearsalOperations:   executionReadRouteOperations,
	...
	paths := []string{
		...
		"/api/v1/execution/orders/missing",
		"/api/v1/execution/orders/unknown/events",
	}
	for index, path := range paths {
		response := executionReadRehearsalRequest(t, proxyServer.URL+path, "execution-read-wire-"+string(rune('a'+index)))
		body, err := io.ReadAll(response.Body)
		_ = response.Body.Close()
		if err != nil {
			t.Fatalf("read execution rehearsal response: %v", err)
		}
		snapshotsMu.Lock()
		...
		assertExecutionReadWire(t, path, response, body, want, false)
	}

	for index, operation := range executionReadRouteOperations {
		path := executionReadOperationPath(operation)
		errorResponse := executionReadRehearsalRequest(
			t, proxyServer.URL+path+"?rehearsalFailure=error", "execution-read-error-"+string(rune('a'+index)),
		)
		...
		timeoutResponse := executionReadRehearsalRequest(
			t, proxyServer.URL+path+"?rehearsalFailure=timeout", "execution-read-timeout-"+string(rune('a'+index)),
		)
	...
	for index, operation := range executionReadRouteOperations {
		path := executionReadOperationPath(operation)
		crashResponse := executionReadRehearsalRequest(t, proxyServer.URL+path, "execution-read-crash-"+string(rune('a'+index)))
	...
	for index, path := range paths {
		response := executionReadRehearsalRequest(t, restartedServer.URL+path, "execution-read-wire-"+string(rune('a'+index)))
		body, _ := io.ReadAll(response.Body)
		_ = response.Body.Close()
		snapshotsMu.Lock()
		want := snapshots[path]
		snapshotsMu.Unlock()
		assertExecutionReadWire(t, path, response, body, want, true)
	}
}

func executionReadHeaderSnapshot(header http.Header) map[string][]string {
	snapshot := make(map[string][]string, len(executionReadWireHeaders))
	for _, name := range executionReadWireHeaders {
		snapshot[name] = append([]string(nil), header.Values(name)...)
	}
	return snapshot
}

func assertExecutionReadWire(t *testing.T, path string, response *http.Response, body []byte, want executionReadWireSnapshot, normalizeBody bool) {
	t.Helper()
	gotBody, wantBody := string(body), want.body
	if normalizeBody {
		gotBody = normalizeExecutionReadEnvelope(body)
		wantBody = normalizeExecutionReadEnvelope([]byte(want.body))
	}
	...
	got := executionReadHeaderSnapshot(response.Header)
	for _, name := range executionReadWireHeaders {
...
func normalizeExecutionReadEnvelope(body []byte) string {
	var value any
	if err := json.Unmarshal(body, &value); err != nil {
		return string(body)
	}
	normalizeExecutionReadValue(value)
...
func normalizeExecutionReadValue(value any) {
	...
	case map[string]any:
		for key, child := range current {
			...
			normalizeExecutionReadValue(child)
		}
	case []any:
...
func executionReadRehearsalRequest(t *testing.T, target, requestID string) *http.Response {
	t.Helper()
	request, err := http.NewRequestWithContext(context.Background(), http.MethodGet, target, nil)
	if err != nil {
		t.Fatalf("create execution read request: %v", err)
	}
	request.Header.Set("X-Request-ID", requestID)
	response, err := http.DefaultClient.Do(request)
	if err != nil {
		t.Fatalf("call execution read route: %v", err)
	}
	return response
}

func executionReadOperationPath(operation string) string {
	switch operation {
	case "GET /api/v1/execution/orders":
		return "/api/v1/execution/orders"
	case "GET /api/v1/execution/orders/{internalOrderId}":
		return "/api/v1/execution/orders/missing"
	case "GET /api/v1/execution/orders/{internalOrderId}/events":
		return "/api/v1/execution/orders/unknown/events"
	default:
		panic("unknown execution read operation: " + operation)
	}
}
