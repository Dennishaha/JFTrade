...
func TestMarketProfilesEndpoint(t *testing.T) {
	...
	var envelope struct {
		...
		Data struct {
			...
			Markets       []marketdataapp.MarketProfileDTO `json:"markets"`
		} `json:"data"`
	}
	...
	byCode := make(map[string]marketdataapp.MarketProfileDTO, len(envelope.Data.Markets))
	...
	descriptor, err := marketdataapp.FutuProviderDescriptor(t.Context())
	if err != nil {
		t.Fatalf("marketdataapp.FutuProviderDescriptor: %v", err)
	}
...
func TestNormalizeMarketInstrumentEndpoint(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			var envelope struct {
				OK   bool                                            `json:"ok"`
				Data marketdataapp.NormalizeMarketInstrumentResponse `json:"data"`
			}
			if err := json.NewDecoder(resp.Body).Decode(&envelope); err != nil {
				t.Fatalf("decode response: %v", err)
			}
