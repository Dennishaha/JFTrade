...
func TestNormalizeManagedBrokerAccountAppliesDefaults(t *testing.T) {
	account := settingsfile.NormalizeManagedBrokerAccount(jfsettings.ManagedBrokerAccount{
		BrokerID:           "  ",
		AccountID:          "  12345678  ",
		DisplayName:        "  ",
		TradingEnvironment: "  ",
...
func TestNormalizeFutuConfigAppliesDefaults(t *testing.T) {
	config := settingsfile.NormalizeFutuConfig(jfsettings.FutuIntegrationConfig{UseEncryption: true})
	...
	if config.APIPort != 11110 {
		t.Fatalf("apiPort = %d", config.APIPort)
	}
	if config.WebSocketPort != 11111 {
		t.Fatalf("webSocketPort = %d", config.WebSocketPort)
	}
	if config.MaxWebSocketConnections != 20 {
...
func TestNormalizeExecutionSettingsAppliesDefaultsAndBounds(t *testing.T) {
	settings := settingsfile.NormalizeExecutionSettings(jfsettings.ExecutionSettings{
	...
	settings = settingsfile.NormalizeExecutionSettings(jfsettings.ExecutionSettings{DefaultTradingEnvironment: "paper"})
