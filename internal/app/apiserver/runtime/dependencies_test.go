...
func TestCheckNodeRuntimeDependencyUsesMacOSCommonPathFallback(t *testing.T) {
	...
	if got := ResolveNodeRuntime(jftsettings.PineWorkerSettings{}); got != "/opt/homebrew/bin/node" {
		t.Fatalf("ResolveNodeRuntime() = %q, want shared fallback", got)
	}
}
...
func TestRuntimeDependenciesAggregatesRequiredStatus(t *testing.T) {
	...
	result := Dependencies(context.Background(), jftsettings.PineWorkerSettings{})
...
func TestNodeRuntimeDependencyMessagesHandleInvalidAndTruncatedOutput(t *testing.T) {
	...
	if got := CheckNodeRuntimeDependency(context.TODO(), jftsettings.PineWorkerSettings{}); got["status"] != runtimeDependencyStatusOK {
