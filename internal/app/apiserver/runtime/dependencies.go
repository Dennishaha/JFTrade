...
const (
	...
	pineWorkerRuntimeEnv               = "JFTRADE_PINEWORKER_RUNTIME"
)
...
// OverrideDependencyProbe replaces process lookup and command execution for
// contract tests in packages that consume runtime dependency diagnostics.
// The returned function restores the previous probe.
func OverrideDependencyProbe(
...
func CheckNodeRuntimeDependency(ctx context.Context, settings jftsettings.PineWorkerSettings) map[string]any {
	return checkNodeRuntimeDependency(ctx, settings)
}

func ResolveNodeRuntime(settings jftsettings.PineWorkerSettings) string {
...
func nodeRuntimeCandidates(settings jftsettings.PineWorkerSettings) []nodeRuntimeCandidate {
	...
	if value := settingsfile.NormalizeNodeBinaryPath(envValue(pineWorkerRuntimeEnv)); value != "" {
		return []nodeRuntimeCandidate{{path: value, source: "env:" + pineWorkerRuntimeEnv}}
	}
