...
const validPineScriptForADKToolFailure = `//@version=6
strategy("Tool failure strategy", overlay=true)
fast = ta.sma(close, 2)
if close > fast
    strategy.entry("long", strategy.long)`

func TestADKToolFailuresPreserveBusinessErrorContracts(t *testing.T) {
	ctx := t.Context()
	...
	t.Run("research and optimization stop on data-readiness or queue errors", func(t *testing.T) {
		...
		if _, err := research.Handler(ctx, map[string]any{"script": validPineScriptForADKToolFailure}); err == nil || !strings.Contains(err.Error(), "research data source unavailable") {
		...
		research, _ = registry.Get("strategy.research_backtest")
		if _, err := research.Handler(ctx, map[string]any{"script": validPineScriptForADKToolFailure}); err == nil || !strings.Contains(err.Error(), "research queue unavailable") {
			t.Fatalf("research queue error = %v", err)
		}

		store, err := assistanttestkit.NewStore(filepath.Join(t.TempDir(), "adk.db"), filepath.Join(t.TempDir(), "secrets"), filepath.Join(t.TempDir(), "skills"))
		if err != nil {
