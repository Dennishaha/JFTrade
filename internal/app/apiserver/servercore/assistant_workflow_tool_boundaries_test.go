...
func TestAssistantWorkflowToolManagerAvailableAndUnavailablePaths(t *testing.T) {
	ctx := t.Context()
	unavailable := newAssistantWorkflowToolManager(nil)
	...
	_, err = unavailable.SaveWorkflow(ctx, "workflow", assistant.WorkflowDefinitionWriteRequest{})
	...
	_, err = unavailable.SaveWorkflowTrigger(ctx, "workflow", "trigger", assistant.WorkflowTriggerWriteRequest{})
	...
	agent, err := serverADKTestStore(t, server).SaveAgent(ctx, assistant.AgentWriteRequest{
		ID: "manager-agent", Name: "Manager Agent", ProviderID: testADKProviderID,
		Status: assistant.AgentStatusEnabled, WorkMode: assistant.WorkModeChat,
	})
	if err != nil {
		t.Fatal(err)
	}
	manager := newAssistantWorkflowToolManager(server)
	payload := assistant.WorkflowDefinitionWriteRequest{
		ID: "manager-workflow", Name: "Manager Workflow", Status: assistant.WorkflowStatusEnabled,
		AgentID: agent.ID, WorkMode: assistant.WorkModeChat, PromptTemplate: "Review {{ .symbol }}",
		DefaultInputs: map[string]any{"symbol": "US.AAPL"},
		CanvasGraph: &assistant.WorkflowCanvasGraph{
			Version: "adk-workflow-canvas/v1",
			Nodes: []assistant.WorkflowCanvasNode{
				{ID: "start", Type: "start"},
				{ID: "agent:primary", Type: "agent"},
				{ID: "monitor", Type: "monitor"},
			},
			Edges: []assistant.WorkflowCanvasEdge{
	...
	if page, err := manager.ListWorkflows(ctx, assistant.WorkflowStatusEnabled, 10, 0); err != nil || page.Total == 0 || len(page.Items) == 0 {
	...
	trigger, err := manager.SaveWorkflowTrigger(ctx, workflow.ID, "", assistant.WorkflowTriggerWriteRequest{
		ID: "manager-trigger", Type: assistant.WorkflowTriggerTypeWebhook, Title: "Manager Trigger", Status: assistant.WorkflowTriggerStatusEnabled,
	})
	if err != nil {
...
func TestADKAdaptersExecuteValidDownstreamContracts(t *testing.T) {
	...
	watchlistTool := assistantassembly.NewWatchlistToolAdapter(server.watchlistSvc)
	if _, err := watchlistTool.List(ctx, WatchlistListInput{Group: "missing-group"}); err == nil || !strings.Contains(err.Error(), "not found") {
	...
	definition, err := server.stores.Design.SaveDefinition(stratsrv.Definition{
		...
		Name:         "Adapter Contract Backtest",
		Version:      "0.1.0",
		...
		Interval:     "1m",
		Script: `//@version=6
strategy("Adapter Contract Backtest", overlay=true)
strategy.entry("Long", strategy.long, qty=1)`,
	...
	research, err := server.adkStartResearchBacktest(ResearchBacktestInput{
		Script: `//@version=6
strategy("Research Contract", overlay=true)
strategy.entry("Long", strategy.long, qty=1)`,
