...
func TestOrderUpdateMappingsRetainBrokerLifecycleFields(t *testing.T) {
	price := 101.25
	amount := 250.0
	payout := 75.0
	filledQuantity := 1.0
	...
	fromBroker := broker.OrderSnapshot{
		...
		Remark: stringPointerOrNil("lifecycle"), LastError: stringPointerOrNil("none"), TimeInForce: stringPointerOrNil("DAY"), Currency: stringPointerOrNil("USD"),
	}
	...
	updates.ApplyFill(t.Context(), "futu", fill)
	updated, ok := server.stores.ExecutionOrders.Order(orders[0].InternalOrderID)
	if !ok || updated.Payout == nil || *updated.Payout != payout {
		t.Fatalf("prediction payout was not persisted on parent order = %#v", updated)
	}
	if events := server.stores.ExecutionOrders.Events(orders[0].InternalOrderID).Events; len(events) != 2 {
