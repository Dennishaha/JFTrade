...
// PortfolioReconciliationPosition portfolio 持仓对账条目。
type PortfolioReconciliationPosition struct {
	BrokerID               string   `json:"brokerId" binding:"required"`
	TradingEnvironment     string   `json:"tradingEnvironment" binding:"required"`
	AccountID              string   `json:"accountId" binding:"required"`
	Market                 string   `json:"market" binding:"required"`
	...
	SymbolName             *string  `json:"symbolName"`
	Status                 string   `json:"status" binding:"required"`
	ProjectedQuantity      *float64 `json:"projectedQuantity"`
	BrokerQuantity         float64  `json:"brokerQuantity" binding:"required"`
	QuantityDelta          float64  `json:"quantityDelta" binding:"required"`
	ProjectedAveragePrice  *float64 `json:"projectedAveragePrice"`
	BrokerAverageCostPrice *float64 `json:"brokerAverageCostPrice"`
	AveragePriceDelta      *float64 `json:"averagePriceDelta"`
	ProjectedRealizedPnl   *float64 `json:"projectedRealizedPnl"`
	BrokerRealizedPnl      *float64 `json:"brokerRealizedPnl"`
	RealizedPnlDelta       *float64 `json:"realizedPnlDelta"`
	ProjectedUpdatedAt     *string  `json:"projectedUpdatedAt"`
}

// PortfolioReconciliationResponse GET /portfolio/{id}/reconciliation 响应。
type PortfolioReconciliationResponse struct {
	BrokerReadStatus
	Positions []PortfolioReconciliationPosition `json:"positions" binding:"required"`
}

func portfolioReconciliationReadError(err error) *PortfolioReconciliationResponse {
	return &PortfolioReconciliationResponse{
		BrokerReadStatus: readErrorStatus(err),
		Positions:        []PortfolioReconciliationPosition{},
	}
}

// PortfolioCashReconciliationBalance portfolio 现金对账条目。
type PortfolioCashReconciliationBalance struct {
	BrokerID                      string   `json:"brokerId" binding:"required"`
	TradingEnvironment            string   `json:"tradingEnvironment" binding:"required"`
	AccountID                     string   `json:"accountId" binding:"required"`
	Currency                      string   `json:"currency" binding:"required"`
	Status                        string   `json:"status" binding:"required"`
	ProjectedCashBalance          *float64 `json:"projectedCashBalance"`
	BrokerCash                    *float64 `json:"brokerCash"`
	CashDelta                     float64  `json:"cashDelta" binding:"required"`
	BrokerAvailableWithdrawalCash *float64 `json:"brokerAvailableWithdrawalCash"`
	BrokerNetCashPower            *float64 `json:"brokerNetCashPower"`
	ProjectedUpdatedAt            *string  `json:"projectedUpdatedAt"`
}

// PortfolioCashReconciliationResponse GET /portfolio/{id}/cash-reconciliation 响应。
type PortfolioCashReconciliationResponse struct {
	BrokerReadStatus
	Balances []PortfolioCashReconciliationBalance `json:"balances" binding:"required"`
}

func portfolioCashReconciliationReadError(err error) *PortfolioCashReconciliationResponse {
	return &PortfolioCashReconciliationResponse{
		BrokerReadStatus: readErrorStatus(err),
		Balances:         []PortfolioCashReconciliationBalance{},
	}
}
