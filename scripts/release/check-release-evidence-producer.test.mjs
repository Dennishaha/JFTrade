...
const binder = fs.readFileSync("scripts/release/bind-release-evidence.mjs", "utf8");
const schema = fs.readFileSync(
  "tests/fixtures/release/release-evidence-payload-binding.schema.json",
  "utf8",
);
...
test("payload verifier rejects the legacy commit argument spelling", () => {
  const result = spawnSync(process.execPath, [
    "scripts/release/verify-release-evidence-payload.mjs",
...
test("payload verifier rejects unsafe workflow and artifact identifiers", () => {
  const script = "scripts/release/verify-release-evidence-payload.mjs";
  const baseArgs = [
    "--root", "/tmp",
