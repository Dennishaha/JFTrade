...
const sidecarDir = join(rootDir, "workers/marketdata-sidecar");
const specPath = join(sidecarDir, "marketdata-sidecar.spec");
...
const python =
  process.env.JFTRADE_MARKETDATA_BUILD_PYTHON?.trim() ||
  ...
  (existsSync(virtualenvPython)
    ...
    : process.platform === "win32"
      ...
      : "python3");
const genericDryRun =
  process.env.JFTRADE_MARKETDATA_ASSET_BUILD_DRY_RUN?.trim();
const dryRun = genericDryRun
  ? genericDryRun === "1"
  : process.env.JFTRADE_YFINANCE_ASSET_BUILD_DRY_RUN === "1";
assertBuildPython(python);
const binaryBase = `marketdata-sidecar-${targetGOOS}-${targetGOARCH}`;
...
const tempDir = mkdtempSync(join(tmpdir(), "jftrade-marketdata-build-"));
...
try {
  ...
  console.log(`Building market-data sidecar -> ${outputDir}`);
  if (dryRun) {
    console.log(`DRY RUN ${formatCommand(python, args)}`);
  } else {
    run(python, args, {
      ...process.env,
      JFTRADE_MARKETDATA_BINARY_NAME: binaryBase,
    ...
    console.log(`Staged market-data sidecar at ${outputPath}`);
  }
} finally {
...
function assertSupportedTarget(goos, goarch) {
  if (!["darwin", "linux", "windows"].includes(goos)) {
    fail(`Unsupported market-data sidecar target OS: ${goos}`);
  }
  if (!["amd64", "arm64"].includes(goarch)) {
    fail(`Unsupported market-data sidecar target architecture: ${goarch}`);
  }
}
...
function assertBuildPython(command) {
  ...
  if (result.error) {
    fail(`Could not execute market-data build Python ${JSON.stringify(command)}: ${result.error.message}`);
  }
  if (result.status !== 0) {
    const detail = String(result.stderr || result.stdout || "").trim();
    fail(
      `Could not inspect market-data build Python ${JSON.stringify(command)}` +
  ...
  try {
    ...
    fail(`Could not parse market-data build Python version from ${JSON.stringify(command)}.`);
  }
  ...
  if (
    ...
    fail(
      `market-data PyInstaller builds require CPython 3.14.x; ` +
        `detected ${payload?.implementation || "unknown"} ${detected} at ${JSON.stringify(command)}. ` +
        `Recreate workers/marketdata-sidecar/.venv with Python 3.14 or set ` +
        `JFTRADE_MARKETDATA_BUILD_PYTHON to a Python 3.14 executable.`,
    );
  }
}
