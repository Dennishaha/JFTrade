...
const defaultPolicyPath = fileURLToPath(new URL("./workspace-architecture-policy.json", import.meta.url));
...
export function validateLayoutPolicy(policy, metadata, options = {}) {
  ...
  for (const declaration of policy.packages ?? []) {
    ...
    for (const field of ["path", "status", "layer"]) {
  ...
  for (const pkg of workspacePackages) {
    ...
    if (!declaration) {
      errors.push(`workspace package ${pkg.name} is not registered in workspace-architecture-policy.json`);
      continue;
    }
...
if (pathToFileURL(path.resolve(process.argv[1] ?? "")).href === import.meta.url) {
  ...
  if (errors.length > 0) {
    ...
    console.log("Rust workspace architecture policy passed.");
  }
}
