...
export function assistantRuntimeFixtureRoot(root = repositoryRoot) {
  return path.join(root, "tests/fixtures/compatibility/assistant-runtime");
}

export function runAssistantRuntimeProcess(command, args, options = {}) {
...
export function assertAssistantRuntimeEquivalent(rustOutput, expected) {
  assert.deepEqual(
    rustOutput,
    expected,
    "Assistant runtime output differs from the pinned compatibility contract",
  );
...
export function runAssistantRuntimeReplay(root = repositoryRoot) {
  const expected = JSON.parse(fs.readFileSync(
    path.join(assistantRuntimeFixtureRoot(root), "assistant-rig-corpus.expected.json"),
    "utf8",
  ));
  const rustOutput = runAssistantRuntimeReference(root);
  assertAssistantRuntimeEquivalent(rustOutput, expected);
  return {
...
if (pathToFileURL(path.resolve(process.argv[1] ?? "")).href === import.meta.url) {
  const result = runAssistantRuntimeReplay();
  console.log(
    `Assistant runtime compatibility replay passed: ${result.statuses} statuses, ` +
