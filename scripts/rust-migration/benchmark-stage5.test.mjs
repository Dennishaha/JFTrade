...
import {
  evaluateStage5Performance,
  summarizeStage5Samples,
} from "./benchmark-stage5.mjs";

test("stage 5 samples report tail latency and maximum RSS", () => {
  const summary = summarizeStage5Samples([
...
test("stage 5 gates reject latency and RSS regressions independently", () => {
  const baseline = { elapsedMillis: { p95: 100 }, peakRssBytes: 1000 };
  assert.deepEqual(
    evaluateStage5Performance(baseline, {
  ...
  const failed = evaluateStage5Performance(baseline, {
