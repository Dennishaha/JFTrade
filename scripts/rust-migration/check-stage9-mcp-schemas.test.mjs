...
test("Stage 9 MCP schema checker runs the Go reference and Rust deep-equality replay", () => {
  assert.deepEqual(
    stage9MCPCommands.map(({ executable }) => executable),
    ["go", "cargo"],
  );
  assert.match(stage9MCPCommands[0].args.join(" "), /TestStage9MCPToolSchemasMatchGoReference/);
  assert.match(
    stage9MCPCommands[1].args.join(" "),
...
test("Stage 9 MCP schema checker is executable", () => {
  ...
  assert.match(result.stdout, /all 69 Go input schemas deep-match Rust descriptors/);
});
