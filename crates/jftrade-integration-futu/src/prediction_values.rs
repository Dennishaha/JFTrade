...
pub(super) fn event_value(
    item: crate::trade_proto::qot_get_event_contract_event_list::EventItem,
) -> Value {
    json!({
...
pub(super) fn contract_value(
    item: crate::trade_proto::qot_get_event_contract::ContractItem,
) -> Value {
    json!({
...
pub(super) fn milestone_value(
    item: crate::trade_proto::qot_get_event_contract_milestone_list::MilestoneItem,
) -> Value {
    json!({
...
pub(super) fn snapshot_value(
    item: crate::trade_proto::qot_get_event_contract_snapshot::SnapshotItem,
) -> Value {
    json!({
...
pub(super) fn order_book_value(
    item: crate::trade_proto::qot_get_event_contract_order_book::OrderBookItem,
) -> Value {
    let levels =
        |items: Vec<crate::trade_proto::qot_get_event_contract_order_book::OrderBookLevel>| {
            items
                .into_iter()
...
pub(super) fn kline_value(
    item: crate::trade_proto::qot_get_event_contract_kline::KlineItem,
) -> Value {
    json!({
        "code": security_value(&item.code),
...
pub(super) fn ticker_value(
    item: crate::trade_proto::qot_get_event_contract_ticker::TickerItem,
) -> Value {
    json!({
        "code": security_value(&item.code),
...
pub(super) fn combo_event_value(
    item: crate::trade_proto::qot_get_event_contract_combo_list::ComboEvent,
) -> Value {
    json!({
        "eventSecurity": security_value(&item.event_security),
        "eventName": item.event_name,
...
impl ComboQuoteRequest {
    pub(super) fn parse(value: &Value) -> Result<Self, PredictionMarketReadError> {
        ...
        let mvc = object
            ...
            .map(str::trim)
            .filter(|v| !v.is_empty())
            .ok_or_else(|| invalid("mvc is required"))?;
        ...
        let legs = object
            ...
            .ok_or_else(|| invalid("legs are required"))?;
        ...
        for leg in legs {
            let item = leg
                .as_object()
                .ok_or_else(|| invalid("prediction combo leg is invalid"))?;
            let instrument = item
                .get("instrumentId")
                .and_then(Value::as_str)
                .ok_or_else(|| invalid("prediction combo leg instrumentId is required"))?;
            ...
            let side = item
                ...
                .transpose()
                .map_err(|_| invalid("prediction combo leg side must be BUY or SELL"))?
                .unwrap_or(1);
            let pred_side = item
                .get("predictionSide")
                .and_then(Value::as_str)
                ...
                .transpose()
                .map_err(|_| invalid("predictionSide must be YES or NO"))?
                .unwrap_or(1);
            let ratio = item.get("ratio").and_then(Value::as_i64).unwrap_or(1);
            if !(1..=100).contains(&ratio) {
                return Err(invalid(
                    "prediction combo leg ratio must be between 1 and 100",
                ));
            }
