...
pub trait RemoteWatchlistWritePort: Send + Sync + std::fmt::Debug {
    fn modify(&self, group_name: &str, operation: &str, securities: &[Value])
...
impl FutuRemoteWatchlistReader {
    ...
    fn call(&self, protocol: u32, body: Vec<u8>) -> Result<Vec<u8>, CustomizationError> {
        ...
        Ok(coordinator.session()?.managed_session().call(protocol, &body)?)
    }
}
...
impl RemoteWatchlistReadPort for FutuRemoteWatchlistReader {
    fn groups(&self) -> Result<Vec<Value>, CustomizationError> {
        ...
        .encode_to_vec();
        let response = wire::Response::decode(self.call(PROTO_GET_USER_SECURITY_GROUP, body)?.as_slice())?;
    ...
    fn members(&self, group_name: &str) -> Result<Vec<Value>, CustomizationError> {
        ...
        if group_name.is_empty() {
            return Err(CustomizationError::Invalid("group name is required".to_owned()));
        }
        ...
        .encode_to_vec();
        let response = wire::Response::decode(self.call(PROTO_GET_USER_SECURITY, body)?.as_slice())?;
        ensure_success(response.ret_type, response.ret_msg)?;
        Ok(response
            .s2c
            .map(|s| {
...
impl RemoteWatchlistWritePort for FutuRemoteWatchlistReader {
    fn modify(&self, group_name: &str, operation: &str, securities: &[Value]) -> Result<Value, CustomizationError> {
        let group_name = group_name.trim();
        if group_name.is_empty() {
            return Err(CustomizationError::Invalid("group name is required".to_owned()));
        }
        let op = match operation.trim().to_ascii_lowercase().as_str() {
            ...
            "delete" | "remove" => 2,
            "move_out" | "moveout" => 3,
            _ => return Err(CustomizationError::Invalid("operation must be add, delete, or move_out".to_owned())),
        };
        ...
        .encode_to_vec();
        let response = wire::Response::decode(self.call(PROTO_MODIFY_USER_SECURITY, body)?.as_slice())?;
        ensure_success(response.ret_type, response.ret_msg)?;
...
pub trait AlertCustomizationReadPort: Send + Sync + std::fmt::Debug {
    ...
    fn option_events(&self, count: i32, page: Option<&str>) -> Result<Vec<Value>, CustomizationError>;
}
...
impl AlertCustomizationReadPort for FutuAlertQuery {
    fn price(&self, market: Option<&str>) -> Result<Vec<Value>, CustomizationError> {
        ...
        .encode_to_vec();
        let response = wire::Response::decode(self.call(PROTO_GET_PRICE_REMINDER, body)?.as_slice())?;
        ensure_success(response.ret_type, response.ret_msg)?;
        Ok(response.s2c.map(|s| s.price_reminder_list.into_iter().flat_map(price_entries).collect()).unwrap_or_default())
    }
    ...
    fn option_events(&self, count: i32, page: Option<&str>) -> Result<Vec<Value>, CustomizationError> {
        ...
        let body = wire::Request {
            c2s: wire::C2s { count: Some(count), page: page.map(str::to_owned) },
        }
        .encode_to_vec();
        let response = wire::Response::decode(self.call(PROTO_GET_OPTION_EVENT_ALERT, body)?.as_slice())?;
        ensure_success(response.ret_type, response.ret_msg)?;
        Ok(response.s2c.map(|s| s.alert_list.into_iter().map(option_event_entry).collect()).unwrap_or_default())
    }
}
...
impl AlertCustomizationWritePort for FutuAlertWrite {
    fn set_price(&self, payload: &Value) -> Result<Value, CustomizationError> {
        ...
        let price = payload
            ...
            .ok_or_else(|| CustomizationError::Invalid("price must be a positive finite number".to_owned()))?;
        let enabled = payload
            .get("enabled")
        ...
        let body = wire::Request { c2s: wire::C2s {
            security,
            op,
            key,
            r#type: parse_optional_i32(payload.get("type"), "type")?,
            freq: parse_optional_i32(payload.get("frequency").or_else(|| payload.get("freq")), "frequency")?,
            value: Some(price),
            note: payload.get("note").and_then(Value::as_str).map(str::to_owned),
            reminder_session_list: Vec::new(),
            header: None,
        }}.encode_to_vec();
        let response = wire::Response::decode(self.call(PROTO_SET_PRICE_REMINDER, body)?.as_slice())?;
        ensure_success(response.ret_type, response.ret_msg)?;
    ...
    fn set_option_event(&self, payload: &Value) -> Result<Value, CustomizationError> {
        ...
        let body = wire::Request { c2s: wire::C2s { oper_type: operation, alert_list } }.encode_to_vec();
        let response = wire::Response::decode(self.call(PROTO_SET_OPTION_EVENT_ALERT, body)?.as_slice())?;
        ensure_success(response.ret_type, response.ret_msg)?;
...
fn ensure_success(ret_type: i32, message: Option<String>) -> Result<(), CustomizationError> {
    if ret_type == 0 { return Ok(()); }
    Err(CustomizationError::Rejected(message.unwrap_or_else(|| format!("retType={ret_type}"))))
}

fn group_type_label(value: i32) -> &'static str { match value { 1 => "custom", 2 => "system", _ => "unknown" } }
...
fn parse_market(value: &str) -> Option<i32> {
    match value.trim().to_ascii_uppercase().as_str() {
        ...
        value => value.parse::<i32>().ok().filter(|market| !market_label(*market).is_empty()),
    }
}
fn instrument_id(market: i32, code: &str) -> String { format!("{}.{}", market_label(market), code) }
...
fn parse_remote_security(
    ...
    let market = object
        .get("market")
        .ok_or_else(|| CustomizationError::Invalid(format!("securityList[{index}].market is required")))?;
    ...
    .ok_or_else(|| CustomizationError::Invalid(format!("securityList[{index}].market is invalid")))?;
    let code = object
        .get("code")
        ...
        .ok_or_else(|| CustomizationError::Invalid(format!("securityList[{index}].code is required")))?;
...
fn price_reminder_operation(
    ...
    let operation = parse_i32(operation).or_else(|| {
        operation.as_str().and_then(|value| match value.trim().to_ascii_lowercase().as_str() {
            "add" => Some(1),
    ...
    if !(1..=6).contains(&operation) {
        return Err(CustomizationError::Invalid("operation is invalid".to_owned()));
    }
...
fn price_entries(reminder: crate::trade_proto::qot_get_price_reminder::PriceReminder) -> Vec<Value> {
...
fn option_event_entry(item: crate::trade_proto::qot_get_option_event_alert::EventAlertItem) -> Value {
    let underlying = item.underlying.and_then(|s| parse_security_value(s.market, s.code));
    json!({
...
fn parse_security_value(market: i32, code: String) -> Option<Value> { Some(json!({"market": market_label(market), "code": code, "instrumentId": instrument_id(market, &code)})) }
fn format_price_type(value: i32) -> &'static str { match value { 1 => "price_up", 2 => "price_down", _ => "unknown" } }
fn format_frequency(value: i32) -> &'static str { match value { 1 => "once", 2 => "once_a_day", _ => "always" } }

fn security_from_payload(payload: &Value) -> Result<crate::trade_proto::qot_common::Security, CustomizationError> {
    ...
    if code.is_empty() || code.chars().any(char::is_control) {
        return Err(CustomizationError::Invalid("symbol code is invalid".to_owned()));
    }
...
fn option_event_from_payload(payload: &Value) -> Result<crate::trade_proto::qot_get_option_event_alert::EventAlertItem, CustomizationError> {
    ...
    let option_market = match payload.get("optionMarket") {
        None | Some(Value::Null) => None,
        Some(value) => Some(parse_option_market_value(value).ok_or_else(|| {
            CustomizationError::Invalid("optionMarket is invalid".to_owned())
        })?),
    };
    let key = parse_optional_i64(payload.get("key"), "key")?;
    ...
    Ok(crate::trade_proto::qot_get_option_event_alert::EventAlertItem {
        key, enable: enabled,
        option_market, watchlist_group_name: payload.get("watchlistGroupName").and_then(Value::as_str).map(str::to_owned), underlying,
        option_type: payload.get("optionType").and_then(parse_i32), side_type_list: parse_i32_array(payload.get("sideTypeList"), "sideTypeList")?, order_type_list: parse_i32_array(payload.get("orderTypeList"), "orderTypeList")?, market_cap_range: None, expiry_days_range: None, price_range: None, size_range: None, premium_range: None, iv_range: None, earnings_date_begin: payload.get("earningsDateBegin").and_then(Value::as_str).map(str::to_owned), earnings_date_end: payload.get("earningsDateEnd").and_then(Value::as_str).map(str::to_owned), note: payload.get("note").and_then(Value::as_str).map(str::to_owned),
    })
}
...
fn parse_option_underlying(value: &Value) -> Result<crate::trade_proto::qot_common::Security, CustomizationError> {
    let object = value
        .as_object()
...
fn parse_optional_i64(value: Option<&Value>, field: &str) -> Result<Option<i64>, CustomizationError> {
...
fn parse_optional_i32(value: Option<&Value>, field: &str) -> Result<Option<i32>, CustomizationError> {
    let Some(value) = value else { return Ok(None) };
    if value.is_null() {
        return Ok(None);
    }
...
fn parse_option_event_operation(value: Option<&Value>) -> Result<i32, CustomizationError> {
    let value = value.ok_or_else(|| CustomizationError::Invalid("operation is required".to_owned()))?;
    ...
    let operation = operation.ok_or_else(|| CustomizationError::Invalid("operation must be add, delete, modify, enable, disable, or delete_all".to_owned()))?;
    if !(1..=6).contains(&operation) {
        return Err(CustomizationError::Invalid("operation is invalid".to_owned()));
    }
    Ok(operation)
