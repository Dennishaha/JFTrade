...
fn fixture_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../..")
        .join("tests/fixtures/rust-migration/stage5")
}

fn run(input: &Path) -> std::process::Output {
    Command::new(env!("CARGO_BIN_EXE_jftrade-stage5-shadow"))
        .args(["--input", input.to_str().expect("UTF-8 fixture path")])
        .output()
        .expect("stage 5 runner starts")
}

#[test]
fn stage5_shadow_matches_pinned_golden_and_never_dispatches() {
...
#[test]
fn stage5_shadow_rejects_truncated_unknown_and_malformed_fixed_point_input() {
    let root = std::env::temp_dir().join(format!("jftrade-stage5-invalid-{}", std::process::id()));
