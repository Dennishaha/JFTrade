...
fn fixture_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../..")
        .join("tests/fixtures/rust-migration/stage7")
}

fn run(input: &Path) -> std::process::Output {
    Command::new(env!("CARGO_BIN_EXE_jftrade-stage7-shadow"))
        .args(["--input", input.to_str().expect("UTF-8 fixture path")])
        .output()
        .expect("stage 7 runner starts")
}

#[test]
fn stage7_shadow_matches_pinned_golden_and_is_deterministic() {
...
#[test]
fn stage7_shadow_rejects_truncated_unknown_and_incomplete_input() {
    let root = std::env::temp_dir().join(format!("jftrade-stage7-invalid-{}", std::process::id()));
