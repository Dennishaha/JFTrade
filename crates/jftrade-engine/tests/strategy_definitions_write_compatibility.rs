...
fn fixture() -> Fixture {
    let fixture: Fixture = serde_json::from_str(include_str!(
        "../../../tests/fixtures/compatibility/api-transport/strategy-definitions-write.json"
    ))
...
fn seed_go_strategy_schema(path: &std::path::Path) {
    let connection = rusqlite::Connection::open(path).expect("create strategy fixture");
    connection
        .execute_batch(
