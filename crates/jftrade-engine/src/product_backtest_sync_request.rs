...
pub(super) fn parse_sync_request(payload: &Value) -> Result<SyncRequest, BacktestsWritePortError> {
    ...
    let (since, until) = if text("startDate").unwrap_or("").trim() != ""
        || text("endDate").unwrap_or("").trim() != ""
    {
        ...
        (
            since,
            end_date + time::Duration::days(1) - time::Duration::nanoseconds(1),
        )
    } else {
        ...
        let since = text("since")
            ...
            .transpose()
            .map_err(BacktestsWritePortError::BadRequest)?
            .unwrap_or_else(|| until - time::Duration::days(30));
        (since, until)
    };
    if until <= since {
        return Err(BacktestsWritePortError::BadRequest(
...
fn normalize_sync_instrument(
    ...
    let (symbol_market, symbol_code) = symbol
        .split_once('.')
        .map(|(prefix, value)| {
            (
    ...
    if symbol.contains('.') && (symbol_market.is_empty() || symbol_code.is_empty()) {
        return Err(BacktestsWritePortError::BadRequest(
            "symbol must be in MARKET.CODE form".to_owned(),
        ));
    }
    ...
    if !matches!(chosen_market.as_str(), "US" | "HK" | "CN" | "SH" | "SZ") {
        return Err(BacktestsWritePortError::BadRequest(
            "invalid market".to_owned(),
        ));
    }
    if chosen_market == "CN" && !symbol_has_market {
        return Err(BacktestsWritePortError::BadRequest(
            "market CN requires an exchange-qualified symbol like SH.600519 or SZ.000001"
                .to_owned(),
        ));
    }
    let symbol_has_code = !symbol_code.is_empty();
    let bare_symbol = if symbol_has_market {
        String::new()
    } else {
        symbol.to_ascii_uppercase()
    };
    let chosen_code = if symbol_has_code {
        symbol_code.clone()
    } else if !bare_symbol.is_empty() {
        bare_symbol.clone()
    } else {
        code.to_owned()
    };
    if chosen_code.is_empty() {
        return Err(BacktestsWritePortError::BadRequest(
            "symbol or code is required".to_owned(),
        ));
    }
    if (!code.is_empty() && symbol_has_code && code != symbol_code)
        || (!code.is_empty() && !symbol_has_code && !bare_symbol.is_empty() && code != bare_symbol)
    {
        return Err(BacktestsWritePortError::BadRequest(
    ...
    let prefix = if symbol_has_market {
        symbol_market
    } else {
        chosen_market.clone()
    };
    let resolved_market = if matches!(prefix.as_str(), "SH" | "SZ") {
        "CN".to_owned()
    } else {
        chosen_market
    };
...
fn markets_match(requested: &str, qualified: &str) -> bool {
    requested == qualified || (requested == "CN" && matches!(qualified, "SH" | "SZ"))
}

fn parse_market_date(
    value: &str,
    market: &str,
) -> Result<time::OffsetDateTime, BacktestsWritePortError> {
    let format =
        time::format_description::parse_borrowed::<1>("[year]-[month]-[day]").map_err(|_| {
            BacktestsWritePortError::BadRequest("invalid date, use YYYY-MM-DD".to_owned())
        })?;
    ...
    let local_date = jiff::civil::Date::new(
    ...
    .map_err(|_| BacktestsWritePortError::BadRequest("invalid date, use YYYY-MM-DD".to_owned()))?;
    let zone = jiff::tz::TimeZone::get(timezone)
        .map_err(|error| BacktestsWritePortError::BadRequest(error.to_string()))?;
    let timestamp = local_date
        .to_zoned(zone)
        .map_err(|error| BacktestsWritePortError::BadRequest(error.to_string()))?
        .timestamp();
...
fn plan_sync_intervals(symbol: &str, requested: &[String], session_scope: &str) -> Vec<String> {
    ...
    for interval in requested {
        ...
        if !planned.iter().any(|value| value == normalized) {
            planned.push(normalized.to_owned());
        }
    }
    planned
}
