...
#[derive(Clone)]
pub struct CompatibilityTradingPort {
    shadow: Arc<Mutex<ShadowTrading>>,
    planned_at: WireTimestamp,
}

pub struct TradingStrategyReplay {
    shadow: Arc<Mutex<ShadowTrading>>,
    planned_at: WireTimestamp,
}

impl TradingStrategyReplay {
    ...
    pub fn strategy(&self, mode: ExecutionMode) -> StrategyCoordinator<CompatibilityTradingPort> {
        StrategyCoordinator::new(
            mode,
            CompatibilityTradingPort {
    ...
    ) -> Result<T, TradingStrategyReplayError> {
        let shadow = self
            .shadow
            .lock()
            .map_err(|_| TradingStrategyReplayError::Poisoned)?;
        Ok(operation(&shadow))
    }
    ...
    pub fn checkpoint(&self) -> Result<ShadowCheckpoint, TradingStrategyReplayError> {
        self.with_shadow(ShadowTrading::checkpoint)
    }
}

impl TradePlannerPort for CompatibilityTradingPort {
    fn plan_trade(&mut self, intent: TradeIntent) -> Result<TradePlanReceipt, String> {
        ...
        let mut shadow = self
            .shadow
            .lock()
            .map_err(|_| "compatibility trading replay lock is poisoned".to_owned())?;
        let plan = shadow
...
#[derive(Debug, Error)]
pub enum TradingStrategyReplayError {
    #[error("compatibility trading replay state lock is poisoned")]
    Poisoned,
}
...
mod tests {
    ...
    use super::TradingStrategyReplay;

    #[test]
    fn composition_maps_strategy_through_a_non_dispatching_trading_port() {
        let assembly = TradingStrategyReplay::new(
            RiskConfig {
