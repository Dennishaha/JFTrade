impl ProductApi {
    fn is_registered_write_path(&self, method: &str, path: &str) -> bool {
        is_market_data_subscription_mutation_path(method, path)
            || (is_brokers_write_path(method, path) && self.write_ports.brokers.is_some())
    }

    fn dispatch_registered_write(&self, request: &ApiRequest) -> Result<ApiOutput, ApiFailure> {
