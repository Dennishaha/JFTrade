...
use jftrade_engine::assistant_runtime_compatibility::AssistantRuntimeReplay;
...
async fn main() {
    if let Err(error) = execute().await {
        eprintln!("jftrade-assistant-runtime-replay: {error}");
        std::process::exit(1);
    }
}
...
async fn execute() -> Result<(), Box<dyn Error>> {
    ...
    let input: AssistantRuntimeInput = serde_json::from_slice(&std::fs::read(input_path)?)?;
    let statuses = input
        .statuses
        .iter()
    ...
    println!(
        "{}",
        serde_json::to_string(&AssistantRuntimeOutput {
...
fn parse_input(arguments: impl Iterator<Item = String>) -> Result<PathBuf, Box<dyn Error>> {
    ...
    match (arguments.next().as_deref(), arguments.next()) {
        ...
        _ => Err("usage: jftrade-assistant-runtime-replay --input <fixture.json>".into()),
    }
}
