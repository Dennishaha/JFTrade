...
use jftrade_engine::stage5::Stage5Assembly;
...
fn main() {
    if let Err(error) = execute() {
        eprintln!("jftrade-stage5-shadow: {error}");
        std::process::exit(1);
    }
}
...
fn execute() -> Result<(), Box<dyn Error>> {
    ...
    let input: Stage5Input = serde_json::from_slice(&std::fs::read(input_path)?)?;
    let assembly = Stage5Assembly::new(input.risk_config, input.planned_at);
    let statuses = input
        .status_cases
        .into_iter()
    ...
    println!(
        "{}",
        serde_json::to_string(&Stage5Output {
...
fn run_strategy(assembly: &Stage5Assembly, scenario: StrategyScenario) -> Value {
...
fn parse_input(mut arguments: impl Iterator<Item = String>) -> Result<PathBuf, Box<dyn Error>> {
    let Some(flag) = arguments.next() else {
        return Err("usage: jftrade-stage5-shadow --input <path>".into());
    };
