...
use jftrade_engine::stage4::Stage4Assembly;
...
fn main() {
    if let Err(error) = execute() {
        eprintln!("jftrade-stage4-shadow: {error}");
        std::process::exit(1);
    }
}
...
fn execute() -> Result<(), Box<dyn Error>> {
    ...
    let input: Stage4Input = serde_json::from_slice(&std::fs::read(input_path)?)?;
    let providers = input
        .providers
        .into_iter()
    ...
    let mut assembly = Stage4Assembly::new(
        providers,
        HelperClientConfig {
    ...
    println!(
        "{}",
        serde_json::to_string(&Stage4Output {
...
fn run_futu(assembly: &mut Stage4Assembly, fixture: FutuFixture) -> Result<Value, Box<dyn Error>> {
...
fn parse_input(arguments: impl Iterator<Item = String>) -> Result<PathBuf, Box<dyn Error>> {
    ...
    match (
        ...
        _ => Err("usage: jftrade-stage4-shadow --input <path>".into()),
    }
}
