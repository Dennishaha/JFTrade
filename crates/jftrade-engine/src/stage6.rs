...
impl Stage6Assembly {
    ...
    pub async fn execute_turn<P: CompletionPort>(
        ...
        max_attempts: usize,
        now: WireTimestamp,
    ) -> Result<Stage6TurnOutcome, Stage6AssemblyError> {
        ...
        for attempt in 1..=max_attempts {
            match provider.complete(input.clone()).await {
                ...
                Err(failure) => {
                    ...
                    return Err(Stage6AssemblyError::Provider(failure));
                }
            }
        }
