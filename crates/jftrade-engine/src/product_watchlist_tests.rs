    ...
    fn from_fixture(fixture: &WatchlistReadFixture) -> Self {
        Self {
            responses: fixture
                .cases
                .iter()
                .filter_map(|case| case.data.clone().map(|data| (case.request_path.clone(), data)))
                .collect(),
        }
    }
}
...
async fn watchlist_read_routes_match_group_fixture_in_cutover_only() {
    ...
    let config = ProductConfig::test_cutover("127.0.0.1:0".parse().expect("address"), &settings_path)
        .expect("config")
        .with_watchlist_read_snapshot_port(Arc::new(FixtureWatchlistReadPort::from_fixture(&fixture)));
    let handle = start_product(config).await.expect("start product");
    assert_eq!(handle.startup_record().owned_routes, 54);
    let address = handle.startup_record().address;
    for case in &fixture.cases {
        assert_eq!(case.method, "GET", "case {}", case.name);
        let (status, response) = request_json_with_status(address, &case.method, &case.request_path, None, &[]).await;
        ...
        if let Some(expected) = &case.data {
            ...
            assert_eq!(response["error"]["code"].as_str(), case.error_code.as_deref(), "case {}", case.name);
            assert_eq!(response["error"]["message"].as_str(), case.error_message.as_deref(), "case {}", case.name);
        }
    }
...
async fn watchlist_read_routes_fail_closed_when_snapshot_port_is_unavailable() {
    ...
    for path in [
        ...
        assert_eq!(response["error"]["code"], "WATCHLIST_UNAVAILABLE", "path {path}");
    }
    handle.shutdown().await.expect("shutdown product");
}
...
async fn watchlist_read_routes_are_not_registered_without_snapshot_port() {
    ...
    let response = request_json(handle.startup_record().address, "GET", "/api/v1/watchlist/groups", None).await;
