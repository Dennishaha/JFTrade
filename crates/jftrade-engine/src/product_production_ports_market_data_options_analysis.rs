...
pub(crate) fn read(
    ...
    if operation == "strategy" {
        return super::product_production_ports_market_data_options_strategy::read(
            Some(runtime),
            path,
            query,
        );
    }
    if operation == "strategy_analysis" {
        return super::product_production_ports_market_data_options_strategy_analysis::read(
            Some(runtime),
            path,
            query,
        );
