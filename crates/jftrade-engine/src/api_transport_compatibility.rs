...
pub struct ApiTransportReplay {
    routes: RouteCatalog,
}

impl ApiTransportReplay {
    pub fn new(routes: Vec<RouteSpec>) -> Result<Self, ApiTransportReplayError> {
        Ok(Self {
    ...
    pub fn evaluate(
        &self,
        input: ApiTransportInput,
    ) -> Result<ApiTransportOutput, ApiTransportReplayError> {
...
impl ApiPort for ApiTransportReplay {
...
#[derive(Debug, Error)]
pub enum ApiTransportReplayError {
...
mod tests {
    ...
    fn assembly_rejects_unregistered_operations() {
        let assembly = ApiTransportReplay::new(vec![RouteSpec {
            method: "GET".to_owned(),
