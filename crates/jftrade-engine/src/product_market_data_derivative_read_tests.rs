...
struct MarketDataDerivativesReadFixture {
    version: String,
    cases: Vec<MarketDataDerivativesReadCase>,
}

#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
struct MarketDataDerivativesReadCase {
    name: String,
    method: String,
    request_path: String,
    expected_status: u16,
...
#[derive(Debug)]
struct FixtureMarketDataDerivativesReadPort {
    responses: BTreeMap<String, Result<Value, MarketDataDerivativeReadSnapshotError>>,
}

impl FixtureMarketDataDerivativesReadPort {
    fn from_fixture(fixture: &MarketDataDerivativesReadFixture) -> Self {
        let responses = fixture
            ...
            .map(|case| {
                let response = match &case.data {
                    ...
                    None => Err(MarketDataDerivativeReadSnapshotError::Failed {
...
impl MarketDataDerivativeReadSnapshotPort for FixtureMarketDataDerivativesReadPort {
    fn read(
        &self,
        path: &str,
        query: &str,
...
#[derive(Debug)]
struct FailingMarketDataDerivativesReadPort;

impl MarketDataDerivativeReadSnapshotPort for FailingMarketDataDerivativesReadPort {
    fn read(
        &self,
        _path: &str,
        _query: &str,
        ...
        Err(MarketDataDerivativeReadSnapshotError::Unavailable(
            "Go derivative catalog snapshot unavailable".to_owned(),
        ))
    }
}
...
#[tokio::test]
async fn market_data_derivatives_read_routes_match_group_fixture_in_cutover_only() {
    let fixture = market_data_derivatives_read_fixture();
    let directory = tempdir().expect("temporary directory");
    let settings_path = directory.path().join("settings.json");
    let config =
        ProductConfig::test_cutover("127.0.0.1:0".parse().expect("address"), &settings_path)
            .expect("config")
            .with_market_data_derivative_read_snapshot_port(Arc::new(
                FixtureMarketDataDerivativesReadPort::from_fixture(&fixture),
            ));
...
#[tokio::test]
async fn market_data_derivatives_read_routes_fail_closed_when_snapshot_port_is_unavailable() {
    let directory = tempdir().expect("temporary directory");
    let settings_path = directory.path().join("settings.json");
    let config =
        ProductConfig::test_cutover("127.0.0.1:0".parse().expect("address"), &settings_path)
            .expect("config")
            .with_market_data_derivative_read_snapshot_port(Arc::new(
                FailingMarketDataDerivativesReadPort,
            ));
    let handle = start_product(config).await.expect("start product");
    for path in [
...
#[tokio::test]
async fn market_data_derivatives_read_routes_are_not_registered_without_snapshot_port() {
    let directory = tempdir().expect("temporary directory");
    let settings_path = directory.path().join("settings.json");
    let config =
        ProductConfig::test_cutover("127.0.0.1:0".parse().expect("address"), &settings_path)
