...
impl ExecutionOrderStore {
    ...
    pub fn get_sequence(&self, name: &str) -> Result<i64, ExecutionOrderStoreError> {
        let connection = self.lock()?;
    ...
    pub fn set_sequence(&self, name: &str, value: i64) -> Result<(), ExecutionOrderStoreError> {
        let connection = self.lock()?;
        connection
            .execute(
                "INSERT INTO execution_sequences (name, value) VALUES (?1, ?2)
    ...
    pub fn list_events_after(
        ...
        after_id: i64,
        limit: usize,
    ) -> Result<Vec<(i64, StoredExecutionOrderEventRecord)>, ExecutionOrderStoreError> {
        let connection = self.lock()?;
        let mut statement = connection
            .prepare(
                "SELECT rowid, id, internal_order_id, event_type, previous_status,
                        next_status, payload_json, created_at
                 FROM execution_order_events
        ...
        let rows = statement
            .query_map(params![after_id, limit as i64], |row| {
                ...
                let record = StoredExecutionOrderEventRecord {
                    id: row.get(1)?,
                    internal_order_id: row.get(2)?,
                    event_type: row.get(3)?,
                    previous_status: row.get(4)?,
                    next_status: row.get(5)?,
    ...
    pub fn list_active_orders_for_instance(
        &self,
        instance_id: &str,
    ) -> Result<Vec<StoredExecutionOrder>, ExecutionOrderStoreError> {
        let connection = self.lock()?;
        query_active_orders_for_instance(&connection, instance_id)
    }
}
#[path = "execution_order_records.rs"]
mod execution_order_records;
#[path = "execution_order_test_cutover.rs"]
mod execution_order_test_cutover;

use execution_order_records::*;
pub use execution_order_test_cutover::*;
