...
func TestAnalyzeProfilesExcludesOnlyFutuTestSupportPackage(t *testing.T) {
	profiles := parseTestProfile(t, `mode: set
github.com/jftrade/jftrade-main/internal/integration/futu/client.go:1.1,2.1 4 1
github.com/jftrade/jftrade-main/internal/integration/futu/testkit/server.go:1.1,2.1 5 0
github.com/jftrade/jftrade-main/internal/integration/futu/testkitx/server.go:1.1,2.1 3 1
`)
	analysis, err := analyzeProfiles(profiles)
	require.NoError(t, err)

	assert.Equal(t, coverageStats{covered: 7, total: 12}, analysis.raw)
	assert.Equal(t, coverageStats{covered: 7, total: 7}, analysis.business)
	assert.Equal(t, scopeCoverage{
		scope:         "internal/integration/futu",
		domain:        "futu",
		coverageStats: coverageStats{covered: 4, total: 4},
	}, coverageForScope(t, analysis.critical, "internal/integration/futu"))
	assert.Equal(t, scopeCoverage{
		scope:         "internal/integration/futu/testkitx",
		domain:        "futu",
		coverageStats: coverageStats{covered: 3, total: 3},
	}, coverageForScope(t, analysis.critical, "internal/integration/futu/testkitx"))
	testSupportIndex, excluded := exclusionIndex("internal/integration/futu/testkit/server.go")
	require.True(t, excluded)
	assert.Equal(t, coverageStats{total: 5}, analysis.excluded[testSupportIndex].coverageStats)
}
...
func TestExclusionRulesAreExplicitAndDoNotHideBackendEntrypoints(t *testing.T) {
	tests := []struct {
		...
		{name: "release asset embed wrapper", fileName: "github.com/jftrade/jftrade-main/internal/yfinanceassets/assets_release.go", category: exclusionBuildVariant, excluded: true},
