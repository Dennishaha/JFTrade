...
func TestAnalyzeDiffCoverageExcludesOnlyFutuTestSupportPackage(t *testing.T) {
	profiles := []*cover.Profile{
		{
			FileName: "github.com/jftrade/jftrade-main/internal/integration/futu/testkit/server.go",
			Blocks: []cover.ProfileBlock{{
				StartLine: 2, EndLine: 2, NumStmt: 5, Count: 0,
			}},
		},
		{
			FileName: "github.com/jftrade/jftrade-main/internal/integration/futu/testkitx/server.go",
			Blocks: []cover.ProfileBlock{{
				StartLine: 2, EndLine: 2, NumStmt: 3, Count: 1,
			}},
		},
	}
	runner := &fakeGitRunner{output: `diff --git a/internal/integration/futu/testkit/server.go b/internal/integration/futu/testkit/server.go
--- a/internal/integration/futu/testkit/server.go
+++ b/internal/integration/futu/testkit/server.go
@@ -1 +2 @@
changed
diff --git a/internal/integration/futu/testkitx/server.go b/internal/integration/futu/testkitx/server.go
--- a/internal/integration/futu/testkitx/server.go
+++ b/internal/integration/futu/testkitx/server.go
@@ -1 +2 @@
changed
`}
	...
	analysis, err := analyzeDiffCoverage("/repo", "base", profiles, runner)
	require.NoError(t, err)
	assert.Equal(t, 2, analysis.changedFiles)
	assert.Zero(t, analysis.ordinary.total)
	require.Equal(t, []scopeCoverage{{
		scope:         "internal/integration/futu/testkitx",
		domain:        "futu",
		coverageStats: coverageStats{covered: 3, total: 3},
	}}, analysis.critical)
	assert.Empty(t, analysis.missingProfileFiles)
}

func TestAnalyzeDiffCoverageExcludesFutuTestSupportWithoutProfile(t *testing.T) {
	repoRoot := t.TempDir()
	writeChangedGoSource(t, repoRoot, "internal/integration/futu/testkit/server.go", `package testkit

func Serve() {}
`)
	writeChangedGoSource(t, repoRoot, "internal/integration/futu/testkitx/server.go", `package testkitx
...
`}
	...
	require.NoError(t, err)
	assert.Equal(t, []string{
		"internal/integration/futu/testkitx/server.go",
	}, analysis.missingProfileFiles)
}
