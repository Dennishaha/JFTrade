...
func (r *strategyRuntime) executeOrderStatement(statement *strategyir.OrderStmt, scope *evaluationScope) error {
	...
	quantityMode, ok := parseQuantityMode(statement.QuantityMode)
	if !ok {
...
func parseIndicatorBinding(statement *strategyir.LetStmt) (indicatorBinding, bool, error) {
	...
	case "ma":
		...
		return indicatorBinding{Alias: statement.Name, Kind: "ma", Key: buildMovingAverageKey(averageType, period, timeUnit), Args: []string{averageType, strconv.Itoa(period), timeUnit}}, true, nil
	case "rsi":
		period, err := expectOnePositiveIntArg(statement.Range.StartLine, name, args)
		if err != nil {
	...
	case "macd":
		values, err := expectPositiveIntArgs(statement.Range.StartLine, name, args, 3)
		if err != nil {
			return indicatorBinding{}, false, err
		}
		return indicatorBinding{Alias: statement.Name, Kind: "macd", Key: fmt.Sprintf("macd:%d:%d:%d", values[0], values[1], values[2]), Args: intsToStrings(values)}, true, nil
	case "kdj":
		values, err := expectPositiveIntArgs(statement.Range.StartLine, name, args, 3)
		if err != nil {
			return indicatorBinding{}, false, err
		}
		return indicatorBinding{Alias: statement.Name, Kind: "kdj", Key: fmt.Sprintf("kdj:%d:%d:%d", values[0], values[1], values[2]), Args: intsToStrings(values)}, true, nil
	case "atr":
		period, err := expectOnePositiveIntArg(statement.Range.StartLine, name, args)
		if err != nil {
			return indicatorBinding{}, false, err
...
func parsePercentage(value string) (float64, error) {
	...
	if err != nil || parsed <= 0 {
		return 0, fmt.Errorf("must be a positive percentage")
	}
	return parsed, nil
}
