...
type strategyRuntime struct {
	...
	syntheticPositionBases       map[string]syntheticPositionBasis
...
func newStrategyRuntime(
	...
	runtime := &strategyRuntime{
		...
		maxDrawdownValue:             normalizeRuntimePositiveFloat(program.Metadata.MaxDrawdownValue),
		maxDrawdownType:              normalizeRuntimeRiskAmountType(program.Metadata.MaxDrawdownType),
		maxDrawdownAlert:             strings.TrimSpace(program.Metadata.MaxDrawdownAlert),
		maxIntradayLossValue:         normalizeRuntimePositiveFloat(program.Metadata.MaxIntradayLossValue),
		maxIntradayLossType:          normalizeRuntimeRiskAmountType(program.Metadata.MaxIntradayLossType),
		maxIntradayLossAlert:         strings.TrimSpace(program.Metadata.MaxIntradayLossAlert),
		maxIntradayFilledOrders:      normalizeRuntimePositiveInt(program.Metadata.MaxIntradayFilledOrders),
		maxIntradayFilledOrdersAlert: strings.TrimSpace(program.Metadata.MaxIntradayFilledOrdersAlert),
		maxPositionSize:              normalizeRuntimePositiveFloat(program.Metadata.MaxPositionSize),
		maxConsLossDays:              normalizeRuntimePositiveInt(program.Metadata.MaxConsLossDays),
		maxConsLossDaysAlert:         strings.TrimSpace(program.Metadata.MaxConsLossDaysAlert),
		pendingOrders:                map[string]pendingOrder{},
		trailingExits:                map[string]trailingExitState{},
		barssinceStates:              map[string]*barssinceState{},
		valuewhenStates:              map[string]*valuewhenState{},
