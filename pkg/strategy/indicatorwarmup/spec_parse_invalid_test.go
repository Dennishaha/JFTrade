...
func TestParseIndicatorRequirementKeysRejectsEveryKeyFamilyBoundary(t *testing.T) {
	tests := []struct {
		key       string
		wantError string
	}{
	...
	for _, tt := range tests {
		t.Run(tt.key, func(t *testing.T) {
			_, err := parseIndicatorRequirementKeys([]string{tt.key})
			if err == nil {
				t.Fatalf("parseIndicatorRequirementKeys(%q) error = nil", tt.key)
			}
...
func TestParseIndicatorRequirementKeysRejectsMalformedVariants(t *testing.T) {
	keys := []string{
		"not-enough",
	...
	for _, key := range keys {
		t.Run(key, func(t *testing.T) {
			if _, err := parseIndicatorRequirementKeys([]string{key}); err == nil {
