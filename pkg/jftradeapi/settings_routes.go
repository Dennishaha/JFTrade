...
func (s *Server) serveSettingsRoutes(w http.ResponseWriter, r *http.Request) bool {
	...
	case r.URL.Path == "/api/v1/settings/execution" && r.Method == http.MethodGet:
		s.writeOK(w, s.store.executionSettings())
	case r.URL.Path == "/api/v1/settings/execution" && r.Method == http.MethodPut:
		s.handleSaveExecutionSettings(w, r)
	case r.URL.Path == "/api/v1/settings/security" && r.Method == http.MethodGet:
		s.writeOK(w, s.store.securitySettings())
	case r.URL.Path == "/api/v1/settings/security" && r.Method == http.MethodPut:
		s.handleSaveSecuritySettings(w, r)
