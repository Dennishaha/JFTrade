...
func TestExecutionCommandErrorUsesStructuredClassification(t *testing.T) {
	status, code := executionCommandError(executionValidationErrorf("order type LIMIT requires price"))
	if status != http.StatusBadRequest || code != "BAD_REQUEST" {
		t.Fatalf("validation error = %d/%s, want 400/BAD_REQUEST", status, code)
	}

	status, code = executionCommandError(broker.NewBrokerError("futu", broker.ErrCodeRateLimited, "too many requests"))
	if status != http.StatusTooManyRequests || code != "BROKER_RATE_LIMITED" {
		t.Fatalf("rate limit error = %d/%s, want 429/BROKER_RATE_LIMITED", status, code)
	}

	status, code = executionCommandError(fmt.Errorf("order quantity exceeds required minimum"))
	if status != http.StatusBadGateway || code != "BROKER_COMMAND_FAILED" {
		t.Fatalf("plain error = %d/%s, want 502/BROKER_COMMAND_FAILED", status, code)
	}
}
