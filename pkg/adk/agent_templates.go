	...
	return []AgentWriteRequest{
		{
			...
			Tools:  []string{"system.status", "tools.search", "workflow.wait", "market.snapshot", "market.candles", "market.depth", "portfolio.summary", "broker.orders", "broker.fills", "risk.state", "strategy.pine_spec", "strategy.validate_pine", "strategy.research_backtest", "backtest.runs", "backtest.result_view", "tasks.create", "tasks.update", "tasks.delete", "tasks.list", "memory.list", "memory.remember", "memory.forget"},
		...
		},
		{
			...
			PermissionMode: PermissionModeApproval, Status: AgentStatusEnabled, MemoryEnabled: true, WorkMode: WorkModeChat, LoopMaxIterations: DefaultLoopMaxIterations,
			Tools:  []string{"tools.search", "workflow.wait", "strategy.definitions", "strategy.pine_spec", "strategy.validate_pine", "strategy.research_backtest", "strategy.save_draft", "strategy.save_definition", "strategy.update_instance_mode", "backtest.runs", "backtest.result_view", "strategy.optimize", "market.snapshot", "market.candles", "tasks.create", "tasks.update", "tasks.delete", "tasks.list", "memory.list", "memory.remember", "memory.forget"},
