...
func TestApprovalPersistenceFailuresRemainObservable(t *testing.T) {
	ctx := context.Background()

	t.Run("async approval rolls back resolution when run staging fails", func(t *testing.T) {
		runtime, run, approvals := newPendingApprovalRun(t, "approval-stage-pending", 2)
		installApprovalRunUpdateRejectTrigger(t, runtime, "reject_stage_pending")
	...
	t.Run("async approval surfaces final transition save failure", func(t *testing.T) {
		runtime, _, approvals := newPendingApprovalRun(t, "approval-stage-final", 1)
		installApprovalRunUpdateRejectTrigger(t, runtime, "reject_stage_final")
	...
	t.Run("synchronous approval does not hide continuation state save failure", func(t *testing.T) {
		runtime, _, approvals := newPendingApprovalRun(t, "approval-sync-save", 1)
		installApprovalRunUpdateRejectTrigger(t, runtime, "reject_sync_continue")
	...
	t.Run("background continuation returns persistence failures to its supervisor", func(t *testing.T) {
		...
		installApprovalRunUpdateRejectTrigger(t, runtime, "reject_background_continue")
...
func TestWorkflowApprovalReconcilerRestagesPersistedResolution(t *testing.T) {
	ctx := context.Background()
	runtime := newTestRuntime(t)
	now := nowString()
...
func newPendingApprovalRun(t *testing.T, runID string, approvalCount int) (*Runtime, Run, []Approval) {
	t.Helper()
	runtime := newTestRuntime(t)
	now := nowString()
