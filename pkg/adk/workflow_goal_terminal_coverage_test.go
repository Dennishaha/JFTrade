...
func TestGoalWorkflowTerminalHelpersPersistCompletionContinuationAndStablePause(t *testing.T) {
	...
	completed, response, done, prompt := executor.finishCompleteGoalWorkflow(ctx, workflowRequest{Session: session}, parent, []Task{task}, openAIChatResult{Reply: "model reply"}, workflowGoalDecisionSnapshot{}, "model reply", 2)
	...
	continued, emptyResponse, paused, nudge := executor.finishContinueGoalWorkflow(ctx, workflowRequest{Session: session}, continuingParent, openAIChatResult{}, workflowGoalDecisionSnapshot{reason: "need one more verification"}, "", 3)
	...
	_, _, snapshot, done, _, _ := executor.resolveGoalWorkflowDecision(ctx, workflowRequest{Session: session}, continuingParent, nil, newBareGoogleADKExecution(continuingParent.ID), decision, openAIChatResult{Reply: "reply"}, "reply", "", 1, false)
	...
	stable, stableResponse, didPause := executor.pauseADKGoalWorkflowIfRequested(ctx, workflowRequest{Session: session}, pausedParent, 1, "")
