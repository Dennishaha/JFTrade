...
from yfinance_sidecar.routes.common import (
    from_yahoo_symbol,
...
async def test_candle_bounds_are_utc_and_forwarded_to_yfinance(
    ...
    response = await client.get(
        ...
        params={
            "period": "1m",
            "from": "2026-07-28T09:30:00-04:00",
            "to": "2026-07-28T09:31:00-04:00",
        },
    )

    assert response.status_code == 200
    assert calls[0]["start"].isoformat() == "2026-07-28T13:30:00+00:00"
    assert calls[0]["end"].isoformat() == "2026-07-28T13:32:00+00:00"
