...
import { useUIColorPreferences } from "@/composables/settings/useUIColorPreferences";
  ...
  <section class="rounded-lg border border-slate-200 bg-white px-5 py-5">
    ...
    <div class="mt-5 grid gap-3 rounded-md border border-slate-200 bg-slate-50 px-4 py-4" :style="previewVars">
      ...
      <div class="grid gap-2 sm:grid-cols-4">
        <div class="rounded border px-3 py-2 text-sm font-semibold text-white jf-swatch-up">买入按钮</div>
        <div class="rounded border px-3 py-2 text-sm font-semibold text-white jf-swatch-down">卖出按钮</div>
      </div>
    </div>
  </section>
</template>
