...
import { runTerminalMessage } from "@/composables/adk/adkChatPresentation";
import {
  formatPermission,
  isInternalSkill,
  permissionModes,
...
import { formatDateTime, formatGenericStatusLabel } from "@/composables/shared/consoleDataFormatting";
...
export function useADKSettingsSectionState() {
  ...
  function applyAgentTemplate(template: Omit<ADKAgent, "createdAt" | "updatedAt">): void {
    agentFormState.agentForm.value = {
      ...
      workMode: template.workMode ?? "chat",
      loopMaxIterations: template.loopMaxIterations ?? 5,
