...
2026-07-29 的 release asset 构建结果：

| 指标 | 实测 | 预算 |
| --- | ---: | ---: |
| 首屏 JavaScript gzip | 403.8 KiB | 444.3 KiB |
| 首屏 CSS gzip | 115.0 KiB | 127.0 KiB |
| 最大异步 JavaScript gzip | 1,448.8 KiB | 1,611.3 KiB |
| 全部控制台 JavaScript gzip | 5,030.6 KiB | 5,468.8 KiB |

最大异步文件是 Monaco TypeScript worker；`editor.api`、Monaco worker、Mermaid core
和 Cytoscape 均不在首屏引用集合中。预算额外禁止这些重依赖进入 `index.html` 的初始
图，即使总体积仍低于阈值也会失败。
