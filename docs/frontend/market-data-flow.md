...
- 盘口深度：`/api/v1/market-data/depth/*`（num 参数控制档数，1-50，默认 10）
- 实时 tick：`/api/v1/ws/live`
- 融资融券参数：`/api/v1/brokers/{brokerId}/margin-ratios`

这些接口都由 sidecar 提供，不是 bbgo 原生 `/api/*`。

盘口深度 REST 拉取当前只做“确保已订阅后再查询”的读侧流程，不复用首推快照；实时盘口推送仍走独立的 order book push 订阅链路，避免推送帧和同步查询响应在同一请求上下文里混淆。
...
| `OrderBookPanel.vue` | 消费盘口深度 API（`/api/v1/market-data/depth/*`），渲染 BBO、买卖档位深度柱状图与档数选择器 |
...
- 改盘口深度档数、显示或轮询策略，先看 `OrderBookPanel.vue` 和 `pkg/futu/adapter.go` 的 `ReadFeatures.orderBook`
