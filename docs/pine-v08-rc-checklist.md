...
- Frontend：Pine editor/intellisense、Pine ↔ visual model、`pineSnippet` 兜底、旧 visual block 只读兼容。
...
- Pine 反解无法标准化的行生成 `pineSnippet`，不得生成新 `codeBlock`。
- 旧 `codeBlock` 和旧合并 `technicalIndicator` 只读兼容；新建路径不暴露旧块。
