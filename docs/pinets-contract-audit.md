...
| Strategy metrics | `buy_and_hold_pnl`, `buy_and_hold_per_gain`, `strategy_outperformance` in PineTS `0.9.31` strategy state | Captured as optional `strategy_metrics` with presence flags so zero values remain distinguishable | No dedicated display yet; Go backtest metrics remain authoritative for JFTrade results. |
