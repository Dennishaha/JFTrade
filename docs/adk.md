...
- HITL：需要审批的工具使用 ADK `RequestConfirmation` 和 `adk_request_confirmation` 协议。原始 function-call ID 与 confirmation-call ID 持久化到审批记录；只有保留完整 confirmation 上下文的 `PENDING_APPROVAL` run 会在服务重启后继续恢复，普通 `RUNNING` run 会标记为 orphaned/failed。GO-ADK requested-input/interrupt 不再进入待补充输入状态；触发时当前 run 失败并返回 `ADK_INPUT_UNSUPPORTED`。
...
- ADK Go v2 的原生 `skilltoolset` 不会根据 `allowed-tools` 动态安装产品工具；JFTrade 使用 Toolset 请求处理器和 `temp:` invocation state 实现声明门控，保持工具调度、权限与审批仍走原有产品控制面。
...
- Provider tool calling 是工具执行的唯一入口；后端保留权限、审批、审计和投影控制面，并把 ADK Go v2 confirmation 事件投影到 JFTrade Approval 和 timeline；requested-input 事件直接失败为 `ADK_INPUT_UNSUPPORTED`。
