...
- Workflow Agent：当前对外工作模式是 `chat`、`task`、`loop`。`task`/`loop` 会先由内部 Planner 生成结构化 plan；静态 planned child workflow 已具备 ADK Go v2 workflow graph 编译路径，生产 task/loop 仍保留 JFTrade parent/child run facade 与 runtime task toolset，用于维持现有审批恢复和前端投影合同。旧的 `sequential`、`parallel` 请求值不再作为运行模式接收。
...
JFTrade 的非 chat 工作模式正在向 ADK Go v2 workflow graph 收敛，不替代 ADK Go v2 的 Agent、Runner、Session、Tool 或 HITL 执行语义。每个 Agent 可配置默认工作模式，聊天请求也可以临时覆盖。当前后端只接受 `chat`、`task`、`loop`；历史 `sequential`、`parallel` 会在迁移时归一到 `chat`，作为请求覆盖值会被拒绝。
...
Planner 阶段只负责产出结构化 plan，不直接启动业务子智能体，也不暴露给普通用户 agent。后端会校验 `order` 和 `dependsOn` 形成的 DAG，并在落库时把 planner 内部依赖映射为真实 `adk_tasks` 依赖；若 Planner 未调用 `finish`、产物为空、依赖非法、循环依赖或无法映射到当前工作模式，本次 workflow 直接失败并记录原因。静态 planned child workflow 可把已校验 plan 编译为 ADK Go v2 workflow graph，使用普通 edge、fan-in JoinNode 和 AgentNode 表达依赖；动态 task/loop 生产路径仍保留现有 facade，等待 v2 graph 的异步工具审批恢复语义完成回归后再切换。
...
- 动态 workflow planning、parent/child run、审批队列、执行计划和 child view 都是 JFTrade 产品层投影；静态 planned child workflow 已接入 ADK Go v2 workflow graph，动态 task/loop 生产路径仍保留现有 facade。
- Planner 工具是内部控制面工具，用于把用户目标转成可校验的执行 plan，不作为运行期动态创建 agent 或直接调用 child agent 的机制。
- Provider tool calling 是工具执行的唯一入口；后端保留权限、审批、审计和投影控制面，并把 ADK Go v2 confirmation/request-input 事件投影到 JFTrade Approval 和 timeline，但不再本地猜测用户意图并主动插入工具调用。
