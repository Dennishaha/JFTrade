...
- `test:rust`：本地使用固定版本的 cargo-nextest 运行 `--workspace --all-targets --locked`。仓库包装器按平台下载 `0.9.143` 预编译包并验证 SHA-256，因此开发机与 CI 使用同一 runner；Cargo 的 `--all-targets` 本身不包含 doctest，替换 runner 不减少既有测试集合。受影响增量测试、PineTS 发布门禁及桌面跨平台发布前置测试亦统一收敛至该 wrapper。
...
`main` 无条件执行完整核心门禁。Policy、Contracts、Rust Static、Rust Test Build、四个 Rust Tests 分片、Compatibility、Web、Pine 和 Python 按依赖图并行；Rust Tests 只等待共享 archive，Compatibility 先以单个 Cargo 构建序列预编译所需 replay binaries，再直接并行执行七类 replay，避免多个 `cargo run` 争用同一 target lock。Desktop 只依赖自己的 Web、Pine、sidecar 和 contracts 构建输入，不等待 Rust Static 或 Rust Tests。
