...
发布候选的资格工作流为独立的 `desktop-release-qualification.yml`：它必须从目标 `refs/tags/vX.Y.Z` 调度，校验同提交的成功 `desktop-release.yml` run，并以 `desktop-release-candidate-evidence` artifact 输出绑定配置。真实 release evidence 由可信的 `desktop-release-evidence.yml` producer 处理两层 artifact：producer 只下载并校验受信 `evidence_ref` 上游产生的 payload artifact（含真实报告、run/ref/commit 和 GitHub artifact id/digest），再上传明确的 `desktop-release-evidence-bound` binding artifact；`release-evidence-inputs.v2` 的 manifest.artifact 绑定 payload 元数据而非自身输出包，以避免自引用摘要循环。qualification 必须同时校验 producer run/output artifact 与 payload artifact API 元数据，并按 `tests/fixtures/rust-migration/stage9/release-evidence-inputs.schema.json` 的 v2 严格绑定校验真实签名 updater、SBOM/provenance、rollback、backup 和 security 文件；manifest、artifact 元数据和每个文件的 schema/status/binding、摘要、大小、realpath 都必须一致，本地 checker 输出、命令文本和 `external_release_runner_evidence_required` 占位值均不得伪装成 prerequisite。`desktop-release.yml` 的 tag push 不接受隐式或合成候选，只有通过 GitHub API 找到该 tag/commit 的成功 qualification run 并校验其 artifact 后才允许发布。发布完成后的 `desktop-post-release-closeout.yml` 必须在不同于发布 tag 的 evidence ref 上传入完整 release binding、四平台 smoke 报告和摘要清单；摘要清单遵循 `tests/fixtures/rust-migration/stage9/post-release-binding.schema.json`，绑定不一致或复用发布 tag 时立即 fail-closed，只有报告校验通过后才执行完整 closeout checker。
