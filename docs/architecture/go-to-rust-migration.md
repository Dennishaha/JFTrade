...
发布候选的资格工作流为独立的 `desktop-release-qualification.yml`：它必须从目标 `refs/tags/vX.Y.Z` 调度，校验同提交的成功 `desktop-release.yml` run，并以 `desktop-release-candidate-evidence` artifact 输出绑定配置。真实 release evidence 先由固定的 `desktop-release-evidence-intake.yml` 建立 provenance 根：它只接受 allowlist 中唯一的 `desktop-release-evidence-source.yml` 外部 runner artifact，校验 immutable artifact id/digest 及 run/ref/commit，并对 incoming prior-version/four-platform backup 与 independent security review 证据执行 fail-closed 语义检查；报告文件逐字节保留，另行写入 detached source-binding sidecar，不制造 passed 状态。随后 `desktop-release-evidence-payload.yml` 只能消费该 intake artifact，可信的 `desktop-release-evidence.yml` producer 再将报告逐字节复制到固定的 `desktop-release-evidence-payload` staging artifact 并上传 `desktop-release-evidence-bound` binding artifact；`release-evidence-inputs.v2` 的 `manifest.artifact` 绑定 producer staging artifact，`sourceBinding` 保留原始外部 run，避免 payload 自引用摘要循环。qualification 必须同时校验 producer run/output artifact、staging payload 文件字节和所有 GitHub API 元数据，并按 `tests/fixtures/rust-migration/stage9/release-evidence-inputs.schema.json` 的 v2 严格绑定校验真实签名 updater、SBOM/provenance、rollback、backup 和 security 文件；manifest、artifact 元数据和每个文件的 schema/status/binding、摘要、大小、realpath 都必须一致，本地 checker 输出、命令文本和 `external_release_runner_evidence_required` 占位值均不得伪装成 prerequisite。`desktop-release.yml` 的 tag push 不接受隐式或合成候选，只有通过 GitHub API 找到该 tag/commit 的成功 qualification run 并校验其 artifact 后才允许发布。发布完成后的 `desktop-post-release-closeout.yml` 必须在不同于发布 tag 的 evidence ref 上传入完整 release binding、四平台 smoke 报告和摘要清单；摘要清单遵循 `tests/fixtures/rust-migration/stage9/post-release-binding.schema.json`，绑定不一致或复用发布 tag 时立即 fail-closed，只有报告校验通过后才执行完整 closeout checker。
...
- [x] `ProviderRouter` 持有共享 runtime recorder：provider 切换、normalized demand acquire/release/expiry 和 active-provider health 更新会驱动 generation、active count、stream state 与旧 generation 拒绝；recorder 也提供 quote/stream failure-recovery 接口供未来 provider composition 驱动。该 recorder 可由未来 `jftrade-engine` composition 注入 system-status，仍不代表真实 OpenD/provider 已切流。
...
Go transport 按 method + OpenAPI path template 精确选择普通 JSON operation。该中间件位于 desktop token、Web access、Auth、origin 与 CSRF 检查之后；公开 Authorization 始终被替换为私有 Bearer，内部协议标记、已验证 access surface、request ID、query、body 和取消信号一并传递。为让 Rust rehearsal 重放已经由 Go 验证的浏览器会话/CORS/CSRF 决策，Cookie、Origin、Referer 与 `X-CSRF-Token` 也只在受鉴权的 loopback sidecar 边界内转发，不向外部 listener 暴露。Rust sidecar 只在 loopback listener、Bearer、内部协议和 access surface 同时验证成功时接受请求。请求一旦交给 Rust，错误、超时或取消都不会重放 Go handler；SSE、WebSocket、文件及非 JSON 请求继续由 Go 处理。
...
| 2026-08-22 | `adk-read` 组完成 B 档 test-cutover 批量切片 | 24 个 ADK GET 通过 fresh Go testkit store、in-memory session service、完整 snapshot/error fixture 和 Rust replay 覆盖成功投影、分页 query、malformed percent query、动态 ID、资源 404 与 stream error paths；Rust `AdkReadSnapshotPort` 仅在显式 test-cutover 注册，ADK SQLite、Provider、session/runtime、任务/审批/会话写入与 SSE 成功 stream header 仍由 Go 唯一拥有；当前总账为 26 shadow/148 cutover-test-only/104 remaining/0 Rust production owner，成功 SSE corpus/header 兼容仍是 qualification blocker |
  ...
  | 2026-08-23 | `watchlist-write` 组完成 A 档 test-only rehearsal | 八个本地 watchlist mutation route 通过 Go owner fixture（45 cases）、Rust leaf replay、missing binding empty-string/JSON binding/cancellation envelope quirks 和 product raw-response route isolation；仅显式 `WatchlistWritePort` 注册，Go watchlist SQLite、quote/provider/OpenD、import transaction 与副作用仍是唯一 production owner；与 backtests-write 集成后总账更新为 26 shadow/184 cutover-test-only/68 remaining/0 Rust production owner，真实事务恢复、幂等、安全、签名发布和 hard-cut 仍阻塞 qualification |
   ...
   | 2026-08-24 | `research-presets-write` 完成规范化 corpus 与显式 durable adapter | Go 参考测试新增 42 个 `ScreenDefinitionV2` 成功/字段错误案例；`jftrade-research` 依据冻结 catalog fixture 实现对等规范化、默认填充、稳定 ID、参数/union/条件校验并通过 Rust 差分。`jftrade-engine` 新增仅由 `ProductConfig::with_research_preset_sqlite_test_cutover` 可达的 durable adapter，接入既有 schema-validated store 的 CRUD/revision/conflict；默认/生产 profile 不变，三条 route 仍为 test-only，Go 仍是 production owner |
   | 2026-08-24 | `settings-read` 组完成 C 档 qualification | 十一个无副作用 settings GET 通过通用 Go sidecar wire/error/timeout/crash/restart rehearsal 与 Rust 实际产品路由回放；覆盖 ADK、MCP、broker、onboarding、execution、security、system notifications、Pine worker、provider 与 exchange-calendar projections，并验证认证边界和 settings 文件不变。`GET /api/v1/settings/data-management/databases` 因 SQLite/storage 依赖继续留在 Tier B；Go 仍是唯一 production owner，Rust 默认 profile 不启用写入；总账更新为 12 shadow/133 cutover-test-only/133 cutover-qualified/0 remaining/0 Rust production owner |
   | 2026-08-24 | `data-management-read` 完成 B 档 qualification | `GET /api/v1/settings/data-management/databases` 通过既有 compatible/missing/incompatible SQLite Go reference 与 Rust replay、complete/summary/filter/unknown authenticated wire rehearsal、error/timeout/crash fail-closed 和 restart-time Go rollback；Rust 产品测试证明空数据目录读操作不创建 database/marker/sidecar 且 settings 字节不变。所有 maintenance/cleanup mutation 仍为 test-only，Go 仍是唯一 production owner；总账更新为 11 shadow/133 cutover-test-only/134 cutover-qualified/0 remaining/0 Rust production owner |
   | 2026-08-24 | `system-control-read` 完成混合 C/B 档 qualification | OpenD install guide 与七个 real-trade safety-control GET 通过既有 install/real-trade Go reference、Rust populated/malformed fail-closed replay、authenticated wire/error/timeout/crash/restart rehearsal及缺失 control-file 不创建证明；对应风险/硬停/kill-switch mutation 仍为 test-only，Go 仍是唯一 production owner；总账更新为 3 shadow/133 cutover-test-only/142 cutover-qualified/0 remaining/0 Rust production owner |
   | 2026-08-24 | `storage-overview-read` 完成 C 档 qualification | `GET /api/v1/system/storage/overview` 的四个空队列投影通过 Rust authenticated exact-data replay 与 Go sidecar wire/error/timeout/crash/restart rehearsal；Rust 不打开数据库或队列，Go 仍是唯一 production owner；总账更新为 2 shadow/133 cutover-test-only/143 cutover-qualified/0 remaining/0 Rust production owner |
   | 2026-08-24 | `runtime-dependencies-read` 完成 B 档 qualification | `GET /api/v1/system/runtime-dependencies` 修复 Rust 忽略 `pineWorker.nodeBinaryPath` 和报告非 Go `runtime:managed` source 的 composition 差异，恢复 settings/env/PATH-common 候选优先级；共享 Node version corpus、Go missing/fallback/error probe、Rust authenticated settings-path route 与 Go wire/error/timeout/crash/restart rehearsal 全部通过，settings 和 Pine lifecycle 不变。Go 仍是唯一 production owner；总账更新为 1 shadow/133 cutover-test-only/144 cutover-qualified/0 remaining/0 Rust production owner |
   | 2026-08-24 | `system-status-read` 完成 B 档 qualification 审计但保持 shadow | Rust 删除非契约 `migrationOwner` 并对齐 build 平台命名/default、settings persistence、request observability shape、Futu descriptor、idle strategy、calendar wiring 与 message；Rust authenticated route 和 Go wire/error/timeout/crash/restart rehearsal 通过。完整 runtime-resource inventory、live/market-data/request 动态观测、broker/strategy live owner 与 release build metadata 尚未迁移，因此不更新 qualification 计数，Go 仍是唯一 production owner |
   | 2026-08-24 | `system-status-read` 收窄 request-observability blocker | Rust API transport 接管 newest-first 有界 error/slow-request history、750ms threshold、importance filter 与 OpenD health/correlation recorder，并通过共享 Go/Rust populated corpus 和真实 middleware 503 观测测试；OpenD provider 尚未接线，live/market-data、runtime inventory、live broker/strategy 与 release metadata blocker 仍保留，因此总账继续为 1 shadow/133 cutover-test-only/144 cutover-qualified/0 remaining/0 Rust production owner |
   | 2026-08-24 | `system-status-read` 闭合 Rust release metadata 注入 | Tauri build launcher 在任何资产准备前校验正式 `vX.Y.Z` tag，拒绝 `dev`/`v0.0.0`，将同一 version/commit/buildTime 注入 Rust 编译并以最后合并的 inline config 锁定 bundle version；调用方 config 不可覆盖。平台 package/sign/install/upgrade/uninstall/rollback/runtime-smoke closeout gate 仍全部开放，route 继续 shadow，Go/Wails 仍是唯一 production owner |
   | 2026-08-24 | `system-status-read` 接入 Rust market-data recorder | `jftrade-marketdata::ProviderRouter` 现在实际驱动共享 runtime recorder 的 normalized demand generation、provider reconfigure 和 active health；recorder 自身覆盖 quote/stream failure-recovery 与 idempotent close，product adapter 已用该 recorder 做 authenticated projection。真实 OpenD socket、provider lifecycle、poll/push worker 和 production composition 尚未接线，route 继续 shadow，Go 仍是唯一 production owner |
   ...
   | 2026-08-24 | OpenD health 到 ProviderRouter 的显式 seam | `market_data_health_from_probe` 将协议中立 probe 映射为 broker-neutral `HealthStatus`，fixture 证明显式 router 可从 ready 更新到 failed；默认产品仍不创建 ProviderRouter、不连接 OpenD，poll/push/reconnect 和生产 owner 仍未接线 |
   | 2026-08-24 | OpenD subscription lifecycle seam 接入 | `OpenDSubscriptionLifecycle` 组合 normalized demand、physical subscribe/unsubscribe plan、generation fencing、5/10/20/30 秒 action retry、poll/quote/stream recorder recovery 与幂等 close；本地 fixture 覆盖 stale callback 和旧 generation 拒绝，但尚未执行真实 socket/push worker 或默认 product composition |
   | 2026-08-24 | OpenD Qot_Sub action executor 接入 | `OpenDSubscriptionExecutor` 在 deadline-bound persistent TCP session 上复刻 `InitConnect` handshake、Qot_Sub market/security/subtype、Basic/Kline/OrderBook push-registration omission/false 语义及 retType/error mapping；本地 mock listener 覆盖 subscribe/unsubscribe round-trip 和 invalid input。Qot_Update push decode、poll worker、ProviderRouter/default product composition 与生产 owner 仍未接线 |
   | 2026-08-24 | OpenD Qot_Update push frame/decode seam 接入 | `OpenDFrameReader` 可从同一 deadline-bound session 读取 unsolicited frame；`decode_quote_push` 复刻 Go 对 BasicQot/KL/OrderBook 的 protocol、retType、S2C 缺失、proto2 required-field 校验和字段映射语义，`OpenDSubscriptionLifecycle::ingest_quote_push` 只接受 active generation；有效 push 交给 consumer，malformed/rejected/empty push 静默丢弃，连接/重连 recorder 状态由 coordinator 的 authenticated session boundary 驱动，旧 generation/closed frame 不触碰状态；本地 mock/codec tests 通过，poll worker、reconnect orchestration、ProviderRouter/default product composition、真实 OpenD 与生产 owner 仍未接线 |
   ...
   | 2026-08-24 | OpenD managed session 分流 seam 接入 | `OpenDManagedSession` 以单一 reader、并发 pending RPC 表和精确 protocol/serial matching 分流 response 与 generation-tagged unsolicited frame；peer EOF/invalid transport 会唤醒全部 waiter，超时请求移除后迟到 response 不会进入旧 waiter，close 关闭 socket 并幂等等待 reader。mock TCP 覆盖 push/RPC 交错、同 serial 错 protocol、逆序并发响应、EOF、迟到响应和本地关闭；现有 health probe/Qot_Sub executor 尚未切换到该 seam，push consumer、poll/reconnect coordinator、ProviderRouter/default product composition、真实 OpenD 与 production owner 均未接线 |
   | 2026-08-24 | OpenD health/Qot_Sub 统一到 managed session | `OpenDInitializedSession` 在单 reader session 上只执行一次 `InitConnect`，`OpenDTcpProbe` 与 `OpenDSubscriptionExecutor` 共享 authenticated boundary。连接角色保留 Go observable quirk：短命 health probe 发送 `RecvNotify=false`，long-lived subscription data session 发送 `RecvNotify=true`；单连接 mock 通过显式 data-session 配置覆盖 GetGlobalState、BasicQot push 与 Qot_Sub 交错。ProviderRouter/default product composition、真实 OpenD 与 production owner 仍未接线 |
   | 2026-08-24 | OpenD managed event pump seam 接入 | `OpenDSessionEventPump::poll_once` 以 bounded timeout 单步消费 managed-session event，并把 active push 交给 generation-fenced lifecycle；unknown/rejected/empty/malformed push 均按 Go stream handler 静默丢弃，不记录 stream failure，active peer EOF 仅返回一次 `ReconnectRequired`，stale event 与 local/lifecycle-first shutdown 无 failure/reconnect 副作用。该 test-only seam 不启动 thread、不 dial/retry、不重放订阅、不修改 router；reconnect coordinator、ProviderRouter/default product composition、真实 OpenD 与 production owner 仍未接线 |
   | 2026-08-24 | broker-neutral snapshot poll executor seam 接入 | `jftrade-marketdata::SnapshotPollExecutor` 以同步单步 injected query 复刻 Go collector 的 1 秒 cadence、1.5 秒 freshness、recorder-owned 5/10/20/30 秒 retry window、normalized/deduplicated demand、仅 requested tick cache commit、failure 保留旧 cache 和 generation-change completion discard；本地 tests 覆盖 fresh/cadence/retry/success/failure/stale/closed，并固定 non-positive policy values 仍使用 Go default。该 seam 无 timer/task/deadline/transport/provider activation；900ms caller deadline、fallback、reconnect、default composition、真实 OpenD 与 production owner 仍未接线 |
   | 2026-08-24 | OpenD Qot_GetBasicQot 3004 query seam 接入 | `OpenDBasicQuoteExecutor` 在现有 initialized managed session 上发送 canonical sorted/deduplicated SecurityList，复用 BasicQot proto2 required-field decoder，保留 retType/errCode/message 与 incomplete-response error，并按 Go 语义把成功的 absent/empty `S2C` 接受为空结果；lifecycle active BASIC record 与 session/lifecycle generation 双 fence 禁止 poll 隐式订阅或跨连接读取。本地单连接 mock 覆盖 success/rejection/absent/empty/incomplete/subscription-required/generation mismatch。BasicQuote→collector Tick/session timestamp mapper、900ms deadline、recoverable read retry、delayed fallback、reconnect/default composition、真实 OpenD 与 production owner 仍未接线 |
   | 2026-08-25 | OpenD BasicQot 到 collector Tick 的保守 mapper seam 接入 | `OpenDBasicQuoteExecutor::query_ticks` 将 3004 响应映射到当前 broker-neutral Tick（Fixed8 price、arbitrary-precision Decimal volume），保持市场/代码规范化、重复行 last-wins、零价丢弃、caller observation time 与 provider generation，并拒绝 non-finite/out-of-range price 而不触碰 cache/recorder；query 通过 managed pending-RPC 固定 Go collector 的 900ms deadline，timeout 后移除 serial，late response 不会回到旧 waiter。`Tick.volume` 已从 `i64` 收窄修复为 numeric-JSON-compatible `DecimalText` 并通过 fractional `hpVolume` regression 与 Stage 4 differential；完整 extended-session/time corpus、retry/fallback/reconnect/default composition、真实 OpenD 与 production owner 仍未接线 |
   | 2026-08-25 | Rust rehearsal proxy 补齐浏览器认证上下文转发 | `rustrehearsal` 在 Go 已完成 desktop/Web/Auth/origin/CSRF 验证后，继续将 Cookie、Origin、Referer 与 `X-CSRF-Token` 传到受鉴权 loopback Rust sidecar，同时始终以私有 Bearer 覆盖公开 Authorization；新增 browser session/CORS/CSRF header regression。该变化只扩大 test/rehearsal 证据，不改变 route ownership、默认 profile、Go/Wails production owner 或公开 listener 安全边界 |
   | 2026-08-25 | OpenD coordinator 提升为显式 composition seam | `OpenDSessionCoordinator` 移除 `cfg(test)` 限制，公开 generation-fenced topology reconcile、bounded push polling、BasicQot→TickCache snapshot poll 和幂等 close；`ProductRuntimeConfig::with_opend_session_coordinator` 可把同一 recorder 接入 system-status projection，并在 shutdown 关闭 session，同时拒绝与 ProviderRouter 双 owner 组合。默认 desktop profile 仍不设置该字段，不启动 timer/thread、不拨号真实 OpenD、不激活 ProviderRouter，route/Go owner/route ledger 统计不变；后续仍需 runtime task、dynamic demand、reconnect/backoff、live differential、发布恢复和正式 owner gate |
