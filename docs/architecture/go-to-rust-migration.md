...
当前准入状态：执行中，尚未进入删除步骤。`jftrade-engine` 已增加受鉴权的独立 read-only product shadow：真实启动 loopback Axum，当前登记 26 个 GET handler，不登记任何写入或通知副作用 route。UI appearance、onboarding、execution、ADK timeout、security password、MCP 配置/token、system notification、Pine worker、exchange calendar、market-data/backtest Provider 选择与 broker integration/account 的兼容写实现只在临时目录 cutover 测试中启用；security GET 只投影是否已配置密码，绝不序列化 hash，临时 security PUT 逐项兼容端口/Unicode 字符数/字节上限、Argon2id、禁用时强制关闭 public access、listener 失败回滚和 desktop-only 403；MCP GET 只返回脱敏配置并在 Rust listener 尚未接管时明确报告未运行，临时 MCP PUT/reset 已覆盖端口/认证拒绝、一次性 51 字符 token、Argon2id verifier、明文不落盘、消费方 listener port 与失败回滚，但 production catalog 不登记这些 route；onboarding 复用真实 Node prerequisite、账户和 broker 配置状态但不探测 OpenD，其 cutover PUT 已覆盖完成、dismiss、既有 broker 与时间字段规则；OpenD install guide 只返回 websocket key 是否存在；broker settings GET 逐次重读 Go owner 的原子 settings 文件并保留 descriptor、持久化 integration、effective defaults 与 managed accounts 的既有 wire，其中 `websocketKey` 是现有受鉴权公开契约的一部分，只允许原样响应，不得写日志或扩散到 install guide/onboarding/status。broker cutover 测试另覆盖 integration 保存、managed account upsert/update/delete、`|` ID percent decoding、未知字段保留与原子替换，但默认 shadow 不注册这些 mutation routes。market-data provider GET 只读取归一化选择，backtest provider GET 复用 adapter 所有的三组静态能力描述符，两者都不激活 Provider、启动 Python helper 或连接 OpenD；临时 Provider PUT 已 differential 验证精确 ID、非法值拒绝、active 先持久化后激活失败回滚、backtest 先准备后持久化，但 production runtime port 尚未装配，因此不构成 Provider 生命周期切换。exchange-calendar settings GET/临时 PUT 兼容 legacy source ID、字段缺失/显式 `false`、缺省通知和 override 过滤；`jftrade-calendar` 另外已完成 source descriptor/status 的 provider-neutral 纯模型与 Go fixture differential；sources GET 仅在 test-cutover 注入 consumer-owned `CalendarSourceSnapshotPort` 时注册，完整 status GET 仅在注入独立 `CalendarStatusSnapshotPort` 时注册，二者均由 Go manager fixture differential 驱动，默认 shadow 和正式 launcher 均不注册，因而不声称已经接管远端刷新 manager；watchlist membership GET 仅在 test-cutover 注入消费方 `WatchlistMembershipSnapshotPort` 时注册，使用 `jftrade-watchlist` 的严格市场/身份规范化、现有/未知 instrument projection 与 Go SQLite fixture differential，默认 shadow、正式 launcher 与 Go watchlist/SQLite owner 不变；storage overview 保持当前 Go owner 的四组空队列契约。data-management databases GET 已使用从 Go `sqliteschema.Definitions()` 机械冻结的 9 库完整 schema 定义，通过 read-only open、metadata/version、table/column/index/foreign-key/view/trigger、dynamic K-line table、`quick_check` 与 `foreign_key_check` 精确校验，并兼容路径环境覆盖、rebuild marker、容量/可回收空间、清理候选、`summaryOnly` 和 `databaseId`；任何未知表或结构漂移都 fail-closed 为 incompatible，文件缺失只报告 missing，且绝不创建、迁移、修复、compact、backup、rebuild 或清理数据库。cleanup preview 只在临时目录 cutover 测试登记，Rust 只生成并校验候选预览，不成为生产 owner；SQLite adapter 仍只读，cleanup/execute、backup、compact、rebuild 等写入操作既不注册也不执行。新增 7 个 real-trade GET 逐请求只读 Go owner 的同名 JSON 控制文件，完整投影审批占位、硬停止、熔断、限额和分类事件；文件缺失/空白按禁用默认值，读取或解码失败与 Go 一致 fail-closed，且 Rust 不注册任何激活、解除或限额写 route。新增 `GET /api/v1/adk/agent-templates` 与 `GET /api/v1/research/screens/catalog` 仅返回 Go 定义冻结的静态只读 wire，不接触 Provider、SQLite 或 Assistant store；前者使用无需 runtime/store 的 builtin template wire，后者使用 13 组 Go catalog fixture。`route-ownership.json` v2 逐 operation 记录 method、path、capability、implementation status、production owner、Go removal status、依赖和证据；`test:rust:stage9:route-coverage` 从这 278 条记录实时派生 26 个 read-only shadow、22 个 cutover-test-only、230 个 remaining 与 0 个 Rust production owner，禁止在 closeout evidence 中手写计数；未实现的 route group 不登记为已拥有，也不会由通用成功投影伪装。Go/Wails 正式入口、所有生产写入、Web/MCP listener、SQLite 写入、Provider、交易、Assistant、WebSocket 和发布入口继续保持唯一生产 owner。formal closeout 的只读报告命令为 `pnpm run report:rust:stage9:closeout`（当前 open 时仍以 0 退出）；`pnpm run check:rust:stage9:closeout` 是 fail-closed formal close gate，要求 status=closed、278 个 operation 全部 cutover-qualified 且由 Rust production owner 唯一拥有、Go removal 完成、四平台包/签名/安装/升级/卸载/回退/runtime smoke、signed updater、security/SBOM、hard-cut readiness、签名 rollback artifact、backup/restore drill、立即 post-release smoke 与 Go/Wails ownerDeletion 全部 passed；不设置生产观察窗口，也不保留运行时 Go fallback。

首个切片账本：

| 能力 | 当前 Go owner | Rust owner | 切换/回退 | Go 删除条件 | 状态 |
| --- | --- | --- | --- | --- | --- |
| 十一组 system GET、十三组 settings GET、两组静态只读 catalog GET 与一组 watchlist membership GET | `internal/api/system`、`internal/api/settings`、`internal/api/assistant`、`internal/api/productfeatures/research_screen`、`internal/api/watchlist`、`internal/store/settingsfile`、`internal/store/sqliteschema`、`internal/store/watchlist`、`internal/app/apiserver/datamigration`、`internal/trading` | `jftrade-engine::product`、`jftrade-assistant`、`jftrade-research`、`jftrade-watchlist`、`jftrade-trading`、`jftrade-settings`、`jftrade-calendar`、`jftrade-datamanagement`、`jftrade-store-settings-file`、`jftrade-store-sqlite`、Futu/行情 integration adapters | 默认只读 shadow 精确登记 26 个 GET；17 个 settings 写入/preview route、1 个 notification test POST、2 个 calendar status/source snapshot test-cutover route 与 1 个 watchlist membership test-cutover route（合计 22 个 cutover-test-only route）只在临时目录 cutover 测试登记，其余新增聚合仍只读。cleanup preview 只生成/校验候选预览，不写 SQLite、不执行 cleanup；settings shadow 每次读取当前原子文件，real-trade 控制文件和 9 个 SQLite 文件每次请求只读；`jftrade-calendar` 的 source/status provider-neutral 模型做 Go fixture differential，动态 sources/status routes 仅由各自 consumer-owned port 在 test-cutover 中注册；watchlist membership 只由 `WatchlistMembershipSnapshotPort` 在 test-cutover 中注册并严格 fail-closed，Go watchlist/SQLite 仍是 owner；所有生产 settings、SQLite、broker、Provider、Web/MCP listener、激活/解除/限额写入仍由 Go 唯一拥有。正式 launcher 尚未切换，停止 Rust 进程即回退 Go | status 全字段接入真实能力 owner；settings 全字段兼容；calendar manager/runtime port、watchlist SQLite port、Web/MCP listener 与 Provider 生命周期由 Rust 唯一接管；SQLite 各能力写 owner、broker/real-trade 写路径、跨平台原子替换/权限/损坏恢复、hard-cut readiness、签名 rollback artifact 与备份恢复演练完成；Go 路由无消费者 | authenticated read-only shadow、security/MCP/OpenD guide secret redaction、broker settings 既有 authenticated wire、onboarding、Provider 选择/静态 catalog、calendar legacy 与 source/status projection、watchlist membership identity/revision/groups projection、storage 空队列、9 库精确 schema/data-management overview、real-trade JSON/fail-closed/事件分类契约、ADK agent-templates 与 research screen catalog 已完成 Go/Rust fixture differential；UI/onboarding/execution/ADK/security password/MCP config+token/notification/Pine/calendar/Provider 与 broker integration/account、cleanup preview 及 watchlist membership test-cutover route 已实现，Web/MCP/Provider rollback/preparation 的消费方窄 port 已测但 production adapter 未装配；cleanup preview 仍为临时目录 cutover 测试，SQLite adapter 只读且不执行 cleanup/execute/backup/compact/rebuild，settings/watchlist adapter 每次重读，Go 仍是唯一生产写 owner；route ownership gate 现列出 230 个 remaining operation |
| 交易日历 source snapshot test-cutover slice | Go `internal/exchangecalendar.Manager.Sources()`（通过现有 Go manager fixture） | `jftrade-engine::product::CalendarSourceSnapshotPort` 消费方窄 port；`jftrade-calendar` 提供 provider-neutral source projection DTO | 仅当 `ProductConfig::test_cutover` 注入 port 时登记 `GET /api/v1/system/exchange-calendars/sources`；默认 shadow、正式 launcher 和 Go 生产 owner 不变；port 错误返回 503 fail-closed | calendar manager/runtime port、动态 registry/status/cache/health 和恢复 differential 接入后，才可评估 production route；当前只验证 wire/projection 与 test-cutover route | Go manager fixture differential、路由 envelope、port unavailable fail-closed、默认 shadow 不注册；route ownership 当前为 26 shadow/22 cutover-test-only/230 remaining |
| 交易日历完整 status test-cutover slice | Go `internal/exchangecalendar.Manager.Status()`（通过固定 clock/settings/registry/snapshot 的 Go manager fixture） | `jftrade-engine::product::CalendarStatusSnapshotPort` 消费方窄 port；`jftrade-calendar` 提供 provider-neutral status/market/snapshot DTO | 仅当 `ProductConfig::test_cutover` 注入独立 status port 时登记 `GET /api/v1/system/exchange-calendars/status`；默认 shadow、正式 launcher 和 Go 生产 owner 不变；port 错误返回 503 fail-closed | calendar manager/runtime port、动态 registry/status/cache/health、远端刷新与恢复 differential 接入后，才可评估 production route | 完整 status wire、sample schedules、source health、成功 envelope、port unavailable fail-closed、status-only/both-port route isolation 与 Go fixture differential 通过；route ownership 当前为 26 shadow/22 cutover-test-only/230 remaining |
...
Go transport 已具备按 method + OpenAPI path template 精确选择普通 JSON operation 的 rehearsal proxy，但当前 composition 没有启用任何 operation。该中间件位于 desktop token、Web access、Auth、origin 与 CSRF 检查之后，只向 Rust 传递私有 Bearer、内部协议标记、已验证 access surface、request ID、query、body 和取消信号；公开 Cookie、Authorization 与 CSRF 不跨越进程边界。Rust sidecar 只在 loopback listener、Bearer、内部协议和 access surface 同时验证成功时接受请求。请求一旦交给 Rust，错误、超时或取消都不会重放 Go handler；SSE、WebSocket、文件及非 JSON 请求继续由 Go 处理。两条 immutable catalog operation 要到下一原子提交才会显式启用，因此当前 278 个 production owner 仍全部为 Go。
