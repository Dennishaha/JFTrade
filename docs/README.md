...
- Rust calendar persistence 已兼容 Go owner 的 `MARKET/YYYY/source.json`、RFC3339Nano JSON、`0755/0644` 权限与同目录 fsync 原子替换；加载不会创建路径，并会在保留有效 snapshot 的同时逐文件报告遍历、权限、截断和 JSON 损坏。它目前只提供 store 能力与 Go fixture differential，尚未接入 production manager 或扩大 route owner。
