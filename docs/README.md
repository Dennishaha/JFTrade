...
- Rust 迁移：阶段 1 共存 bridge、阶段 2 codec/SQLite 只读 differential 和阶段 3 `conservative-bar-v1` 纯计算 shadow 核心已完成本地证据；它们不接管公开 API、生产数据库、worker、Futu 或桌面发布，Go/Wails 仍是生产事实。完整阶段、目录约束和切换条件见 [architecture/go-to-rust-migration.md](architecture/go-to-rust-migration.md)。
