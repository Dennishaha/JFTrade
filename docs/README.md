...
- Rust calendar manager 已在 fixture source 边界内接入 registry、builtin/manual policy、snapshot restore/cache、source health/backoff、settings reload、start/close/cancel；启动失败会逆序关闭，Close 幂等，持久化失败不会覆盖有效内存 snapshot。当前未复制真实 HTTP Provider，尚未接入正式 launcher 或扩大 route owner。
