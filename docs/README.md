...
- 当前 data-management maintenance 增量只在临时数据库和 `cutover-test-only.v1` 启用：cleanup execute、backup、compact、rebuild 均经过 writer lease、候选指纹或可验证备份 fencing。calendar、read projection、backtest run/sync read 与 strategy-instance-read 组继续只在显式 snapshot port 下登记，最新门禁派生为 26 shadow、83 cutover-test-only、169 remaining、0 Rust production owner；正式 Go/Wails owner 不变。
