...
- 公开 `pinets` 包已按 workspace `package.json` 精确版本安装并记录版本；当前 `pinets@0.9.29` npm license 为 `AGPL-3.0-only`。
- worker contract 已接入 PineTS `0.9.29` 覆盖的 integer division、用户函数历史索引语义，以及 strategy-state 指标：`buy_and_hold_pnl`、`buy_and_hold_per_gain`、`strategy_outperformance`，并通过可选 `strategy_metrics` 返回。
...
当前仓库已通过 workspace 依赖锁定公开 `pinets@0.9.29`，真实非 mock PineTS worker 进程 smoke 已可作为放行依据；完整 strict release gate 已可通过 `pnpm run check:pinets-release` 在 Windows 和 CI 风格环境中直接运行。
