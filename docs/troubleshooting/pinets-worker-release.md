...
- 公开 `pinets` 包已按 workspace `package.json` 精确版本安装并记录版本；当前 `pinets@0.9.31` npm license 为 `AGPL-3.0-only`。
- worker contract 已接入 PineTS `0.9.31` 覆盖的 integer division、用户函数历史索引语义，以及 strategy-state 指标：`buy_and_hold_pnl`、`buy_and_hold_per_gain`、`strategy_outperformance`，并通过可选 `strategy_metrics` 返回。
