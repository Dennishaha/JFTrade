...
- 公开 `pinets` 包已按 workspace `package.json` 精确版本安装并记录版本；当前 `pinets@0.9.30` npm license 为 `AGPL-3.0-only`。
- worker contract 已接入 PineTS `0.9.30` 覆盖的 integer division、用户函数历史索引语义，以及 strategy-state 指标：`buy_and_hold_pnl`、`buy_and_hold_per_gain`、`strategy_outperformance`，并通过可选 `strategy_metrics` 返回。
...
当前仓库已通过 workspace 依赖锁定公开 `pinets@0.9.30`，真实非 mock PineTS worker 进程 smoke 已可作为放行依据；完整 strict release gate 已可通过 `pnpm run check:pinets-release` 在 Windows 和 CI 风格环境中直接运行。
...
`pnpm run build:frontend-assets` 也使用 Node 入口重建 web dist、复制到 `internal/frontendassets/dist`，并调用 Go zip 工具生成 `dist.zip`；`scripts/build-frontend-assets.sh` 保留为兼容转发器。
...
| `JFTRADE_PINEWORKER_INSTANCE_WORKERS` | `10` | 运行实例 worker 进程最大值；运行实例达到上限时启动失败，前端提示到设置中调高“运行实例 Worker 最大值”或停止其他实例。 |
...
| `JFTRADE_PINEWORKER_PROTO` | worker 默认 proto 路径 | 传给 Node worker 的 proto 文件路径。 |
...
| `JFTRADE_NODE_BINARY` | `node` | 仅 build/dev/start 辅助入口使用；指定 Node 可执行文件路径。 |
...
```bash
pnpm run check:pinets-release --allow-blocked
```
...
这个测试会编译 Node worker，以非 mock 模式启动 worker 进程，并通过 `WorkerManager` 发起 localhost gRPC `RunScript`。它要求 `pinets` 已安装；缺少 `pinets` 时测试会失败，表示发布仍被阻塞。命令行或日志中不应出现 `--mock true`。

当前已有的 `JFTRADE_PINEWORKER_PROCESS_SMOKE=1 go test ./pkg/strategy/pineworker -run TestWorkerManagerProcessSmokeWithNodeWorker -v` 只证明 Node worker、进程生命周期和 gRPC 边界可用；它使用 mock executor，不能替代真实 PineTS smoke。
