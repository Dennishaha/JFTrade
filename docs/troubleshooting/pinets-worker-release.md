...
- `npm run build:pineworker` 通过 RollDown 生成平台无关的 `worker.mjs` Node ESM bundle。
...
`npm run build:pineworker` 会先确认 `pinets` 包已安装并输出其 license。未安装 `pinets` 时，脚本会在调用 RollDown 前失败。`scripts/build-pineworker-assets.sh` 仍保留为兼容入口，但会转发到 Node 版 builder。
...
`npm run test:pineworker` 和 worker package 的 `npm test` 通过 Vitest 在 Node 下运行，不需要额外 worker runtime shim。
