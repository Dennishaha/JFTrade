...
当前仓库已通过 workspace 依赖锁定公开 `pinets@0.9.28`，真实非 mock PineTS worker 进程 smoke 已可作为放行依据；完整 strict release gate 已可通过 `pnpm run check:pinets-release` 在 Windows 和 CI 风格环境中直接运行。
