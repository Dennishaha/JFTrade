...
页面异常
  ...
  -> 开发态默认 127.0.0.1:3000，发布态默认 127.0.0.1:6699，GUI 默认 127.0.0.1:6688
  -> 如果 settings.json 的 interfaces 或环境变量改过绑定地址，先按实际配置检查
  -> 如果 3000/6699 不在：查启动模式和启动退出日志
  -> 如果 3000 在但 WS 断开：查 /api/v1/ws/live 和 sidecar
...
- [websocket-connection.md](websocket-connection.md)
