...
> 迁移状态：仓库已完成 Rust 阶段 1 共存基础、阶段 2 codec/SQLite 只读验证和阶段 3 `conservative-bar-v1` 纯计算 shadow 核心；这些实现仍不参与公开 API、生产数据写入、worker 生命周期或桌面发布，Go/Wails 仍是唯一生产 owner。下文继续描述当前生产架构；目标、守则和放行门禁见 [Go → Rust 完整迁移方案](architecture/go-to-rust-migration.md)。
