...
| 2026-06-29 | Hardened real PineTS process smoke startup | In progress; smoke now detects the installed hoisted `pinets` package directly instead of relying on `npm ls --workspaces`, and the Bun worker main module holds the gRPC server open until SIGINT/SIGTERM. Local verification passed `npm run typecheck:pineworker` and `go test ./pkg/strategy/pineworker`; full non-mock smoke still requires Bun on PATH. |
