...
JFTrade 使用一组固定关联字段串联 HTTP、OpenD、行情、回测、PineTS 与 ADK 日志。SystemPage 只保留有限数量的进程内摘要，完整事实仍以结构化日志和运行记录为准。
...
1. 打开 `/system` 的“状态”页，在“链路观测”中确认记录阈值、最近错误、慢请求和 OpenD 调用健康。
...
4. 对 `bt-*` 或 `sync-*` 事件，从 SystemPage 进入 `/backtest`，核对运行状态、同步进度和结果错误。
5. 对 `run-*` / `session-*` 事件，从 SystemPage 进入 `/adk`，核对 timeline、tool call、approval 和 provider；需要持久化事实时查询 `var/jftrade-api/adk.db` 与 `var/jftrade-api/adk-session.db`。
6. OpenD 事件先看 `lastOperation`、失败计数和 `request_id`，再到 SystemPage 的 OpenD 健康检查与工作进程券商页核对连接、订阅和退避状态。
...
- SystemPage 摘要用于定位关联 ID，不替代数据库、任务状态或完整日志。
