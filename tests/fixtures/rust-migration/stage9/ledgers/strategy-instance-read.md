...
All three operations are `cutover-test-only`, `productionOwner=go`, and `goRemovalStatus=retained`. Strategy lifecycle POST/PUT/DELETE routes, definition writes, PineTS, runtime subscriptions, notifications, and SQLite writes remain outside this slice.
