...
Both operations are `cutover-test-only`, `productionOwner=go`, and `goRemovalStatus=retained`. The default read-only shadow catalog does not register either route. OpenD reset (`POST /api/v1/system/futu-opend/manual-retry`) and all real-trade mutations remain outside this read-only slice.
