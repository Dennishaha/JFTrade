...
后续: Add an observable failed-writer assertion or a Go-owned proxy evidence hook, and add dedicated authenticated GET-sidecar timeout/cancellation/restart evidence for the `adk-read` routes before qualification.
...
- The explicit test-cutover composition now composes the `adk-read` replay route with this group and proves retained terminal events, `after` ordering, client disconnect boundary, and restart recovery. The separate `adk-read` Go/Rust SSE corpus now closes header/event-shape compatibility. Qualification still requires observable failed-writer evidence, authenticated GET-sidecar timeout/cancellation/restart evidence, four-platform release, signing, security, recovery, hard-cut, and only then Go/Wails removal gates.
