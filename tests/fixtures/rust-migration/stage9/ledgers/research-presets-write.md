...
- Rust boundary: `product_research_preset_write_port.rs` is a consumer-owned mutation port. It has no SQLite, Provider/OpenD, filesystem, notification, or production route registration. It is usable only after the integration branch adds an explicit test-cutover adapter and route capability.
