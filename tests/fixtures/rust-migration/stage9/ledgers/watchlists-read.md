...
The fixture normalizes only the dynamic response timestamp to `fixture-time`; no observable Go behavior is corrected. The route is `cutover-test-only`, `productionOwner=go`, and `goRemovalStatus=retained`. The default shadow catalog does not register it. Remote watchlist mutation (`POST /api/v1/watchlists/remote`) remains `remaining` and is outside this read-only slice.
