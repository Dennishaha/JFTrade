...
- The four routes remain unregistered in Rust product assembly and remain `remaining` in the ownership ledger. Go remains the only production write owner.
...
- Route coverage after the worker slice: passed, unchanged at `26 shadow / 172 cutover-test-only / 0 qualified / 80 remaining / 0 Rust production owner`.
