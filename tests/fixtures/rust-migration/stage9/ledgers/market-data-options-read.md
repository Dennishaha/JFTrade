...
No Go observable quirk was found after comparing the Go baseline, fixture and Rust replay. All five operations are `cutover-test-only`, `productionOwner=go`, and `goRemovalStatus=retained`. Rust does not open market-data storage, start a helper, connect OpenD, create subscriptions, or register option mutation routes.
